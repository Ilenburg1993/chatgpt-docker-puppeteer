...
if (categories.TS2339.length > 0) {
  ...
  categories.TS2339.forEach(err => {
    ...
    if (match) {
      ...
      const key = type.includes('ConfigurationManager') ? 'ConfigurationManager' :
                  type.includes('ConnectionOrchestrator') ? 'ConnectionOrchestrator' :
                  type.includes('DRIVER_STATES') ? 'DRIVER_STATES' :
                  type.includes('TargetDriver') ? 'TargetDriver' :
                  type;

      if (!missingProps.has(key)) missingProps.set(key, new Set());
      missingProps.get(key).add(prop);
    }
  });
...
if (categories.TS2322.length > 0) {
  ...
  // Agrupa por padrão
  const literalTypes = categories.TS2322.filter(e =>
  ...
  const arrayTypes = categories.TS2322.filter(e => e.message.includes('is not assignable to type') && e.message.includes('[]'));
...
const fixable = categories.TS2554.length + categories.TS2339.length +
                categories.TS2345.length + categories.TS2322.length;
...
console.log(`   Após Fase 3: ~${totalErrors - categories.TS2694.length - categories.TS2345.length - categories.TS2339.length} erros`);
