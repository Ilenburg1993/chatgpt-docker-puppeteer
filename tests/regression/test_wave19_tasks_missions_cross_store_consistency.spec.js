...
test('wave19: stores vNext mantêm índice cruzado task↔mission e sincronização', async () => {
    const tasksStore = await fs.readFile(path.join(process.cwd(), 'src/dashboard-ui/src/stores/tasks_vnext.js'), 'utf8');
    const missionsStore = await fs.readFile(path.join(process.cwd(), 'src/dashboard-ui/src/stores/missions_vnext.js'), 'utf8');
