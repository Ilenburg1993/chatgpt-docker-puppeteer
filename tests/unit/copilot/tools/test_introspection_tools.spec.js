 ...
 * - report_intent_local: registra e retorna confirmação com timestamp
...
const mocks = vi.hoisted(() => ({
    ...
    getSummary: vi.fn(() => ({
        tools: {
            web_fetch_local: { totalCalls: 10, successCount: 9, errorCount: 1 },
...
describe('introspection-tools', () => {
    ...
    const fakeTools = [
        { name: 'web_fetch_local', description: 'Fetch web content' },
    ...
    describe('isToolDisabled / getDisabledTools', () => {
        it('retorna false por default', () => {
            expect(mod.isToolDisabled('web_fetch_local')).toBe(false);
        });
    ...
    describe('list_tools', () => {
        ...
        it('exclui tools desabilitadas', async () => {
            ...
            await /** @type {any} */ (toggle).handler({ toolName: 'web_fetch_local', enabled: false });
    ...
    describe('get_telemetry', () => {
        ...
        it('filtra por toolName', async () => {
            const tool = mod.introspectionTools.find((t) => t.name === 'get_telemetry');
            const result = await /** @type {any} */ (tool).handler({ toolName: 'web_fetch_local' });

            expect(result.topTools.length).toBe(1);
            expect(result.topTools[0].name).toBe('web_fetch_local');
    ...
    describe('report_intent_local', () => {
        it('registra intent e retorna confirmação', async () => {
            const tool = mod.introspectionTools.find((t) => t.name === 'report_intent_local');
            const result = await /** @type {any} */ (tool).handler({
    ...
    describe('toggle_tool', () => {
        ...
        it('habilita uma tool previamente desabilitada', async () => {
            await find().handler({ toolName: 'web_fetch_local', enabled: false });
            const result = await find().handler({ toolName: 'web_fetch_local', enabled: true });
    ...
    describe('get_tool_health', () => {
        ...
        it('filtra por tool_name específico', async () => {
            const result = await find().handler({ tool_name: 'web_fetch_local' });
