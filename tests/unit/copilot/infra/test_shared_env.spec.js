...
describe('infra/shared/env', () => {
    it('usa fallback para inteiros positivos inválidos', () => {
        ...
        expect(readEnvPositiveInt('TEST_POSITIVE_INT', 42, process.env)).toBe(42);
    ...
    it('aceita zero apenas no helper não negativo', () => {
        ...
        expect(readEnvNonNegativeInt('TEST_NON_NEGATIVE_INT', 42, process.env)).toBe(0);
    });

    it('preserva o domínio especial -1/0/>0 quando configurado', () => {
        vi.stubEnv('TEST_INT_AT_LEAST', '-1');
        expect(readEnvIntAtLeast('TEST_INT_AT_LEAST', 42, -1, process.env)).toBe(-1);
        vi.stubEnv('TEST_INT_AT_LEAST', '-2');
        expect(readEnvIntAtLeast('TEST_INT_AT_LEAST', 42, -1, process.env)).toBe(42);
