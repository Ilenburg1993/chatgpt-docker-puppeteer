// @ts-check
/**
 * tests/unit/copilot/infra/test_sqlite_application_schema.spec.js
 *
 * Provas estruturais e funcionais do runtime/migration ledger SQLite agora owned por Infra.
 *
...
const SQLITE_SRC = fs.readFileSync(path.resolve('src/copilot/infra/database/sqlite/better-sqlite3/runtime.js'), 'utf8');
const SQLITE_PRAGMAS_SRC = fs.readFileSync(path.resolve('src/copilot/infra/database/sqlite/pragmas.js'), 'utf8');
const MIGRATIONS_SRC = fs.readFileSync(
    path.resolve('src/copilot/infra/database/sqlite/application/migrations.js'),
    'utf8',
);

// ─── sqlite.js — Structural Tests ──────────────────────────────────────────────

describe('infra/database/sqlite/better-sqlite3 runtime — structural', () => {
    it('expõe somente uma factory de resource instance-owned, sem service locator global', () => {
        assert.ok(SQLITE_SRC.includes('createBetterSqliteApplicationRuntime'));
        for (const forbidden of [
            ...
            assert.ok(!SQLITE_SRC.includes(forbidden), `runtime não deve conter ${forbidden}`);
        }
    });

    it('deve usar WAL journal mode', () => {
        assert.ok(SQLITE_PRAGMAS_SRC.includes('journal_mode = WAL'));
    });

    it('deve aplicar busy_timeout', () => {
        assert.ok(SQLITE_PRAGMAS_SRC.includes('busy_timeout'));
    });

    it('deve aplicar foreign_keys = ON', () => {
        assert.ok(SQLITE_PRAGMAS_SRC.includes('foreign_keys = ON'));
    });

    it('deve aplicar synchronous = NORMAL', () => {
        assert.ok(SQLITE_PRAGMAS_SRC.includes('synchronous = NORMAL'));
    });

    it('não resolve configuração, filesystem ou shutdown processual dentro do driver runtime', () => {
        assert.ok(!SQLITE_SRC.includes('COPILOT_DB_PATH'));
        assert.ok(!SQLITE_SRC.includes('COPILOT_WORKING_DIRECTORY'));
        assert.ok(!SQLITE_SRC.includes("from 'node:fs"));
        assert.ok(!SQLITE_SRC.includes("from 'node:path'"));
        assert.ok(!SQLITE_SRC.includes("process.on('exit'"));
        assert.ok(!SQLITE_SRC.includes('registerShutdownHandler'));
    });

    it('exige dbPath explícito e mantém logger/disposal no escopo da resource', () => {
        assert.ok(SQLITE_SRC.includes('requires an explicit dbPath'));
        assert.ok(SQLITE_SRC.includes('const log = options.log ?? defaultSqliteLog'));
        assert.ok(SQLITE_SRC.includes('[Symbol.dispose]: close'));
        assert.ok(!SQLITE_SRC.includes('ERR_COPILOT_DB_RECONFIGURE_OPEN'));
        assert.ok(!SQLITE_SRC.includes('ERR_COPILOT_DB_UNCONFIGURED'));
    });
});

// ─── migrations.js — Structural Tests ──────────────────────────────────────────

describe('infra/database/sqlite/application/migrations — structural', () => {
...
describe('infra application SQLite — in-memory integration', () => {
    /** @type {import('better-sqlite3').Database | null} */
    let db = null;

    beforeEach(async () => {
        const { default: Database } = await import('better-sqlite3');
        const { adaptBetterSqliteDatabase } = await import('#copilot/infra/internal/database/sqlite/better-sqlite3');
        const { COPILOT_MIGRATIONS } = await import('#copilot/infra/internal/database/sqlite/application');
        ...
        for (const migration of COPILOT_MIGRATIONS) {
            if (typeof migration.upFn === 'function') {
                migration.upFn(adaptBetterSqliteDatabase(db));
