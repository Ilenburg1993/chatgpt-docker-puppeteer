// @ts-check
import assert from 'node:assert/strict';
import { describe, it } from 'vitest';

import { EventBus, bridgeEmitter, createEventBus } from '#copilot/events/runtime';

/**
 * Helper: cria evento mínimo.
 *
 * @param {string} type
 * @param {Record<string, unknown>} [extra]
 * @returns {import('../../../../src/copilot/events/base-events.js').BaseEvent}
 */
function evt(type, extra = {}) {
    return { type, timestamp: Date.now(), ...extra };
}

// ═════════════════════════════════════════════════════════════════════════════
// createEventBus
// ═════════════════════════════════════════════════════════════════════════════

describe('events/runtime › createEventBus', () => {
    it('retorna instância de EventBus', () => {
        const bus = createEventBus();
        assert.ok(bus instanceof EventBus);
    });
});

// ═════════════════════════════════════════════════════════════════════════════
// on / emit
// ═════════════════════════════════════════════════════════════════════════════

describe('events/runtime › on + emit', () => {
    it('handler recebe evento emitido', () => {
        const bus = createEventBus();
        /** @type {unknown[]} */
        const received = [];
        bus.on('test:foo', (e) => {
            received.push(e);
        });
        const e = evt('test:foo');
        bus.emit(e);
        assert.equal(received.length, 1);
        assert.deepEqual(received[0], e);
        assert.notStrictEqual(received[0], e);
    });

    it('handler não recebe evento de tipo diferente', () => {
        const bus = createEventBus();
        /** @type {unknown[]} */
        const received = [];
        bus.on('test:foo', (e) => {
            received.push(e);
        });
        bus.emit(evt('test:bar'));
        assert.equal(received.length, 0);
    });

    it('múltiplos handlers para mesmo tipo', () => {
        const bus = createEventBus();
        let count = 0;
        bus.on('x:y', () => {
            count++;
        });
        bus.on('x:y', () => {
            count++;
        });
        bus.emit(evt('x:y'));
        assert.equal(count, 2);
    });

    it('rejeita eventType vazio', () => {
        const bus = createEventBus();
        assert.throws(() => bus.on('', () => {}), /non-empty string/);
    });

    it('rejeita handler não-função', () => {
        const bus = createEventBus();
        assert.throws(() => Reflect.apply(bus.on, bus, ['x:y', 'nope']), /function/);
    });

    it('rejeita emit sem type', () => {
        const bus = createEventBus();
        assert.throws(() => Reflect.apply(bus.emit, bus, [{}]), /type/);
    });
});

// ═════════════════════════════════════════════════════════════════════════════
// unsubscribe
// ═════════════════════════════════════════════════════════════════════════════

describe('events/runtime › unsubscribe', () => {
    it('on retorna função de unsubscribe', () => {
        const bus = createEventBus();
        let count = 0;
        const unsub = bus.on('a:b', () => {
            count++;
        });
        bus.emit(evt('a:b'));
        assert.equal(count, 1);
        unsub();
        bus.emit(evt('a:b'));
        assert.equal(count, 1);
    });

    it('unsubscribe é idempotente', () => {
        const bus = createEventBus();
        const unsub = bus.on('a:b', () => {});
        unsub();
        unsub(); // não deve lançar
    });
});

// ═════════════════════════════════════════════════════════════════════════════
// once
// ═════════════════════════════════════════════════════════════════════════════

describe('events/runtime › once', () => {
    it('handler é chamado apenas uma vez', () => {
        const bus = createEventBus();
        let count = 0;
        bus.once('x:y', () => {
            count++;
        });
        bus.emit(evt('x:y'));
        bus.emit(evt('x:y'));
        assert.equal(count, 1);
    });

    it('once retorna unsubscribe funcional', () => {
        const bus = createEventBus();
        let count = 0;
        const unsub = bus.once('x:y', () => {
            count++;
        });
        unsub();
        bus.emit(evt('x:y'));
        assert.equal(count, 0);
    });
});

// ═════════════════════════════════════════════════════════════════════════════
// wildcards
// ═════════════════════════════════════════════════════════════════════════════

describe('events/runtime › wildcards', () => {
    it('namespace wildcard session:* captura session:start', () => {
        const bus = createEventBus();
        /** @type {string[]} */
        const types = [];
        bus.on('session:*', (e) => {
            types.push(e.type);
        });
        bus.emit(evt('session:start'));
        bus.emit(evt('session:end'));
        bus.emit(evt('tool:invoke'));
        assert.deepStrictEqual(types, ['session:start', 'session:end']);
    });

    it('catch-all * captura todos os eventos', () => {
        const bus = createEventBus();
        let count = 0;
        bus.on('*', () => {
            count++;
        });
        bus.emit(evt('a:x'));
        bus.emit(evt('b:y'));
        bus.emit(evt('c:z'));
        assert.equal(count, 3);
    });

    it('handler exato + wildcard + catch-all recebem o mesmo evento', () => {
        const bus = createEventBus();
        /** @type {string[]} */
        const log = [];
        bus.on('tool:invoke', () => {
            log.push('exact');
        });
        bus.on('tool:*', () => {
            log.push('wildcard');
        });
        bus.on('*', () => {
            log.push('catchall');
        });
        bus.emit(evt('tool:invoke'));
        assert.deepStrictEqual(log, ['exact', 'wildcard', 'catchall']);
    });
});

// ═════════════════════════════════════════════════════════════════════════════
// middleware
// ═════════════════════════════════════════════════════════════════════════════

describe('events/runtime › middleware', () => {
    it('middleware é chamado antes dos handlers', () => {
        const bus = createEventBus();
        /** @type {string[]} */
        const order = [];
        bus.use((_event, next) => {
            order.push('mw');
            next();
        });
        bus.on('a:b', () => {
            order.push('handler');
        });
        bus.emit(evt('a:b'));
        assert.deepStrictEqual(order, ['mw', 'handler']);
    });

    it('middleware pode bloquear entrega não chamando next()', () => {
        const bus = createEventBus();
        let delivered = false;
        bus.use((_event, _next) => {
            /* não chama next */
        });
        bus.on('a:b', () => {
            delivered = true;
        });
        bus.emit(evt('a:b'));
        assert.equal(delivered, false);
    });

    it('middleware chain é executada em ordem', () => {
        const bus = createEventBus();
        /** @type {number[]} */
        const order = [];
        bus.use((_e, next) => {
            order.push(1);
            next();
        });
        bus.use((_e, next) => {
            order.push(2);
            next();
        });
        bus.use((_e, next) => {
            order.push(3);
            next();
        });
        bus.on('x:y', () => {
            order.push(4);
        });
        bus.emit(evt('x:y'));
        assert.deepStrictEqual(order, [1, 2, 3, 4]);
    });

    it('rejeita middleware não-função', () => {
        const bus = createEventBus();
        assert.throws(() => Reflect.apply(bus.use, bus, ['nope']), /function/);
    });
});

describe('events/runtime › dispatch ownership semantics', () => {
    it('does not mutate the caller event while middleware enriches the owned envelope', async () => {
        const bus = createEventBus();
        const caller = { type: 'owned:event', value: 1 };
        /** @type {any[]} */ const received = [];
        bus.use((event, next) => {
            /** @type {import('../../../../src/copilot/events/base-events.js').BaseEvent & {enriched?:boolean}} */ (
                event
            ).enriched = true;
            return next();
        });
        bus.on('owned:event', (event) => {
            received.push(event);
        });
        await bus.emitAsync(caller);
        assert.equal(Object.hasOwn(caller, 'timestamp'), false);
        assert.equal(Object.hasOwn(caller, 'enriched'), false);
        assert.equal(received[0]?.enriched, true);
        assert.equal(typeof received[0]?.timestamp, 'number');
    });

    it('uses listener snapshots so mutation during dispatch affects only the next event', () => {
        const bus = createEventBus();
        /** @type {string[]} */ const calls = [];
        /** @type {() => void} */ let removeSecond = () => {};
        bus.on('snapshot:event', () => {
            calls.push('first');
            removeSecond();
            bus.on('snapshot:event', () => {
                calls.push('late');
            });
        });
        removeSecond = bus.on('snapshot:event', () => {
            calls.push('second');
        });
        bus.emit({ type: 'snapshot:event' });
        assert.deepEqual(calls, ['first', 'second']);
        calls.length = 0;
        bus.emit({ type: 'snapshot:event' });
        assert.deepEqual(calls, ['first', 'late']);
    });

    it('contains middleware failure and continues the remaining pipeline', async () => {
        const bus = createEventBus();
        /** @type {string[]} */ const calls = [];
        bus.use(() => {
            calls.push('broken');
            throw new Error('middleware failure');
        });
        bus.use((_event, next) => {
            calls.push('after');
            return next();
        });
        bus.on('mw:event', () => {
            calls.push('handler');
        });
        await bus.emitAsync({ type: 'mw:event' });
        assert.deepEqual(calls, ['broken', 'after', 'handler']);
    });

    it('middleware registration returns a disposer', async () => {
        const bus = createEventBus();
        let calls = 0;
        const dispose = bus.use((_event, next) => {
            calls += 1;
            return next();
        });
        await bus.emitAsync({ type: 'mw:dispose' });
        dispose();
        await bus.emitAsync({ type: 'mw:dispose' });
        assert.equal(calls, 1);
    });
});

// ═════════════════════════════════════════════════════════════════════════════
// counters / stats
// ═════════════════════════════════════════════════════════════════════════════

describe('events/runtime › count + stats', () => {
    it('count retorna 0 para evento nunca emitido', () => {
        const bus = createEventBus();
        assert.equal(bus.count('nope:x'), 0);
    });

    it('count incrementa com cada emit', () => {
        const bus = createEventBus();
        bus.emit(evt('a:b'));
        bus.emit(evt('a:b'));
        bus.emit(evt('a:b'));
        assert.equal(bus.count('a:b'), 3);
    });

    it('stats retorna snapshot de contadores', () => {
        const bus = createEventBus();
        bus.emit(evt('a:x'));
        bus.emit(evt('b:y'));
        bus.emit(evt('a:x'));
        const s = bus.stats();
        assert.equal(s['a:x'], 2);
        assert.equal(s['b:y'], 1);
    });

    it('limita cardinalidade dos contadores sem bloquear entrega de eventos', () => {
        const bus = createEventBus();
        let delivered = 0;
        bus.on('*', () => {
            delivered += 1;
        });
        for (let i = 0; i < 1_025; i++) {
            bus.emit(evt(`dynamic:${i}`));
        }
        assert.equal(delivered, 1_025);
        assert.ok(Object.keys(bus.stats()).length <= 1_000);
        assert.equal(bus.count('dynamic:0'), 0);
        assert.equal(bus.count('dynamic:1024'), 1);
    });
});

// ═════════════════════════════════════════════════════════════════════════════
// listenerCount
// ═════════════════════════════════════════════════════════════════════════════

describe('events/runtime › listenerCount', () => {
    it('inicia em 0', () => {
        const bus = createEventBus();
        assert.equal(bus.listenerCount, 0);
    });

    it('incrementa com on', () => {
        const bus = createEventBus();
        bus.on('a:b', () => {});
        bus.on('a:b', () => {});
        bus.on('c:d', () => {});
        assert.equal(bus.listenerCount, 3);
    });

    it('decrementa com unsubscribe', () => {
        const bus = createEventBus();
        const u1 = bus.on('a:b', () => {});
        bus.on('a:b', () => {});
        u1();
        assert.equal(bus.listenerCount, 1);
    });
});

// ═════════════════════════════════════════════════════════════════════════════
// dispose
// ═════════════════════════════════════════════════════════════════════════════

describe('events/runtime › dispose', () => {
    it('limpa listeners, middleware e contadores', () => {
        const bus = createEventBus();
        bus.on('a:b', () => {});
        bus.use((_e, next) => next());
        bus.emit(evt('a:b'));
        bus.dispose();
        assert.equal(bus.listenerCount, 0);
        assert.equal(bus.count('a:b'), 0);
        assert.deepStrictEqual(bus.stats(), {});
    });

    it('emit após dispose é silencioso (não lança)', () => {
        const bus = createEventBus();
        bus.dispose();
        bus.emit(evt('a:b')); // não deve lançar
    });

    it('on após dispose lança', () => {
        const bus = createEventBus();
        bus.dispose();
        assert.throws(() => bus.on('a:b', () => {}), /disposed/);
    });

    it('use após dispose lança', () => {
        const bus = createEventBus();
        bus.dispose();
        assert.throws(() => bus.use((_e, n) => n()), /disposed/);
    });
});

// ═════════════════════════════════════════════════════════════════════════════
// error isolation
// ═════════════════════════════════════════════════════════════════════════════

describe('events/runtime › error isolation', () => {
    it('erro em handler não impede entrega a outros handlers', () => {
        const bus = createEventBus();
        let reached = false;
        bus.on('a:b', () => {
            throw new Error('boom');
        });
        bus.on('a:b', () => {
            reached = true;
        });
        bus.emit(evt('a:b'));
        assert.ok(reached);
    });
});

// ═════════════════════════════════════════════════════════════════════════════
// bridgeEmitter (M-3)
// ═════════════════════════════════════════════════════════════════════════════

describe('events/runtime › bridgeEmitter', () => {
    // Dynamic import to get bridgeEmitter
    /** @type {typeof import('node:events').EventEmitter} */
    let EventEmitter;

    it('carrega bridgeEmitter', async () => {
        const events = await import('node:events');
        EventEmitter = events.EventEmitter;
        assert.ok(typeof bridgeEmitter === 'function');
    });

    it('M-3: re-emite eventos locais no EventBus', async () => {
        const emitter = new EventEmitter();
        const bus = createEventBus();
        /** @type {unknown[]} */
        const received = [];
        bus.on('hub:session:created', (ev) => {
            received.push(ev);
        });

        bridgeEmitter(emitter, bus, { 'session:created': 'hub:session:created' });
        emitter.emit('session:created', { hubSessionId: 'abc' });

        assert.strictEqual(received.length, 1);
        assert.ok(received[0] && typeof received[0] === 'object');
        assert.strictEqual(/** @type {any} */ (received[0]).hubSessionId, 'abc');
        assert.strictEqual(/** @type {any} */ (received[0]).type, 'hub:session:created');
    });

    it('M-3: unbind remove listeners do emitter original', async () => {
        const emitter = new EventEmitter();
        const bus = createEventBus();
        let count = 0;
        bus.on('x:y', () => {
            count++;
        });

        const unbind = bridgeEmitter(emitter, bus, { ev1: 'x:y' });
        emitter.emit('ev1', {});
        assert.strictEqual(count, 1);

        unbind();
        emitter.emit('ev1', {});
        assert.strictEqual(count, 1, 'não deve incrementar após unbind');
    });

    it('M-3: payload primitivo é tratado como objeto vazio', async () => {
        const emitter = new EventEmitter();
        const bus = createEventBus();
        /** @type {unknown[]} */
        const received = [];
        bus.on('test:event', (ev) => {
            received.push(ev);
        });

        bridgeEmitter(emitter, bus, { raw: 'test:event' });
        emitter.emit('raw', 'string-payload');

        assert.strictEqual(received.length, 1);
        assert.strictEqual(/** @type {any} */ (received[0]).type, 'test:event');
    });
});
