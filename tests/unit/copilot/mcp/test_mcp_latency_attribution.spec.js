...
function healthyLocal(overrides = {}) {
    return {
        ...
        originHttpBoundary: {
            ...
            burstCount: 10,
            overlapCount: 0,
            externalGaps: { count: 9, totalMs: 7_200, averageMs: 800, p50Ms: 750, p95Ms: 1_400, p99Ms: 1_400, lastMs: 700, maxMs: 1_400 },
            ...
            lastTransition: null,
            maxTransition: null,
            thresholdsMs: { externalElevated: 3_000, externalHigh: 8_000, internalPressure: 500, internalDegraded: 1_500 },
            note: 'fixture',
        },
        interToolGap: {
            status: 'normal',
...
function aggregateOperational(overrides = {}) {
    return {
        ...
        root: { source: 'status-root', reachable: true, httpStatus: 200, signal: 'operational', durationMs: 10, error: null },
        statusApi: {
            source: 'status-api',
            reachable: true,
            httpStatus: 200,
...
describe('MCP latency attribution', () => {
    it('reconstructs historical burst gaps, preserves concurrency and excludes long idle pauses', () => {
        ...
        assert.deepEqual(history.topTransitions24h, [
            { from: 'tool-a', to: 'tool-b', count: 1, totalGapMs: 1_900, averageMs: 1_900, p50Ms: 1_900, p95Ms: 1_900, maxMs: 1_900 },
            { from: 'tool-e', to: 'tool-f', count: 1, totalGapMs: 1_700, averageMs: 1_700, p50Ms: 1_700, p95Ms: 1_700, maxMs: 1_700 },
            { from: 'tool-b', to: 'tool-c', count: 1, totalGapMs: 900, averageMs: 900, p50Ms: 900, p95Ms: 900, maxMs: 900 },
        ]);
    });
    ...
    it('groups only same-series latency pulses under sanitized experiment labels', () => {
        ...
        const history = summarizeAuditInterToolHistory(
            [
                ...
                pulseEnd(11_300, 'a3'),
                pulseStart(17_300, 'b1', 'hotspot', { latencyNetworkLabel: 'hotspot-cellular', latencyModelLabel: 'sol' }),
                pulseEnd(17_400, 'b1'),
                pulseStart(21_400, 'b2', 'hotspot', { latencyNetworkLabel: 'hotspot-cellular', latencyModelLabel: 'sol' }),
                pulseEnd(21_500, 'b2'),
                pulseStart(25_500, 'b3', 'hotspot', { latencyNetworkLabel: 'hotspot-cellular', latencyModelLabel: 'sol' }),
                pulseEnd(25_600, 'b3'),
            ],
            base + 26_000,
    ...
    it('attributes user-reported slowness to the pre-MCP/upstream zone when local layers are healthy', () => {
        const result = classifyLatencyAttribution(/** @type {any} */ ({
            reportedSlow: true,
    ...
    it('prioritizes Cloudflare degradation over an otherwise healthy upstream picture', () => {
        const result = classifyLatencyAttribution(/** @type {any} */ ({
            ...
            clientSchemaProjectionStale: false,
            local: healthyLocal(),
            cloudflare: healthyCloudflare({ status: 'degraded', haConnections: 2, reasons: ['ha-connections-below-4'] }),
            publicMcpLoopback: healthyPublicLoopback(),
            reachability: healthyReachability(),
            externalStatus: aggregateOperational(),
        }));
    ...
    it('summarizes Cloudflare metrics using the same HA and RPC gates as the post-change policy', () => {
        const healthy = summarizeCloudflareMetrics(/** @type {any} */ ({
            ok: true,
        ...
        const degraded = summarizeCloudflareMetrics(/** @type {any} */ ({
            ok: true,
    ...
    it('never upgrades container reachability into client health authority', () => {
        const summary = summarizeReachability(/** @type {any} */ ([
