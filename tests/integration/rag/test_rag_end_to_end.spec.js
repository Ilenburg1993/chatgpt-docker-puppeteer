...
describe('RAG end-to-end (no Ollama)', () => {
    it('indexes incrementally and can build markdown context', async () => {
        ...
        try {
            ...
            const r1 = /** @type {any} */ (await ragIndex({ root: ws, paths, embeddingsProvider: embeddings, profile: 'full' }));
            assert.ok(r1.scanned_files >= 1);
            assert.strictEqual(r1.skipped_files, 0);
            assert.ok(r1.embedded_chunks >= 1);

            const r2 = /** @type {any} */ (await ragIndex({ root: ws, paths, embeddingsProvider: embeddings, profile: 'full' }));
            ...
            const r3 = /** @type {any} */ (await ragIndex({ root: ws, paths, embeddingsProvider: embeddings, profile: 'full' }));
            ...
            const inc = /** @type {any} */ (await ragIndexChanged({
                root: ws,
                paths,
                changedPaths: ['src.ts'],
            ...
            const { markdown, result } = /** @type {any} */ (await ragAsk({
            ...
            const q = /** @type {any} */ (await ragQuery({
