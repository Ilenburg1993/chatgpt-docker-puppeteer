...
function confirmReasonAndExecute() {
    const reason = reasonInput.value.trim();
    if (!reason) return;
    showReasonModal.value = false;
    if (pendingAction.value?.callback) {
        pendingAction.value.callback(reason);
    }
...
async function createTask() {
    ...
    requestReason('TASK_CREATE', async (reason) => {
        ...
        try {
            ...
            createForm.value = {
                stage: 'READY', mission_id: '', target: 'auto',
                model: '', priority: 5, system_message: '', user_message: '',
            };
...
<template>
    <div class="space-y-5 animate-fade-in">
        ...
        <div
            ...
            class="surface-card p-4 flex flex-col md:flex-row md:items-center gap-3"
        >
            ...
            <div class="flex-1 flex flex-col md:flex-row gap-2">
                ...
                <template v-if="bulkAction === 'set_stage'">
                    <select v-model="bulkStage" class="w-full md:w-48 px-3 py-2 rounded-lg bg-slate-900/60 border border-slate-700/40 text-slate-200 text-sm">
                        <option value="DRAFT">DRAFT</option>
                        <option value="PROPOSED">PROPOSED</option>
                        <option value="READY">READY</option>
                        <option value="REJECTED">REJECTED</option>
                ...
                <template v-else-if="bulkAction === 'set_target'">
                    <select v-model="bulkTarget" class="w-full md:w-48 px-3 py-2 rounded-lg bg-slate-900/60 border border-slate-700/40 text-slate-200 text-sm">
                        <option value="auto">auto</option>
                        <option value="chatgpt">chatgpt</option>
                        <option value="gemini">gemini</option>
                        <option value="claude">claude</option>
                ...
                <template v-else-if="bulkAction === 'set_priority'">
                    <input v-model.number="bulkPriority" type="number" min="0" max="10"
                ...
                <template v-else-if="bulkAction === 'set_execute_after'">
                    <input v-model="bulkExecuteAfterMs" type="number" placeholder="ms (null=agora)"
                ...
                <template v-else-if="bulkAction === 'reassign_mission'">
                    <select v-model="bulkMissionId" class="w-full md:w-72 px-3 py-2 rounded-lg bg-slate-900/60 border border-slate-700/40 text-slate-200 text-sm">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-800/50">
                    ...
                    <tr v-for="t in items" :key="t.id" class="hover:bg-slate-900/30 transition-colors duration-100">
                        ...
                        <td class="p-3 font-mono text-slate-300 cursor-pointer hover:text-blue-400 transition-colors" @click="router.push(`/tasks/${t.id}`)">
                            {{ t.id }}
                        </td>
                        ...
                        <td class="p-3">
                            <div v-if="t.mission_ref?.id" class="text-xs">
                                <button class="font-mono text-cyan-400 hover:text-cyan-300 hover:underline transition-colors"
                        ...
                        <td class="p-3 text-right">
                            <div class="flex justify-end gap-1.5">
                                ...
                                <Button v-if="t.command_caps?.can_retry" variant="ghost" size="sm" class="h-7 px-2 text-xs" @click="quickAction(t.id, 'retry')">Retry</Button>
        ...
        <Modal :open="showCreate" @update:open="showCreate = $event" size="lg">
            ...
            <div class="space-y-4">
                <div>
                    <label class="text-sm text-slate-300 font-medium">User message <span class="text-red-400">*</span></label>
                    <textarea
                        v-model="createForm.user_message" rows="4"
                        class="w-full px-3 py-2 rounded-lg bg-slate-900/60 border border-slate-700/40 text-slate-200 text-sm focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/20"
                ...
                <div>
                    ...
                    <textarea
                        v-model="createForm.system_message" rows="2"
                        class="w-full px-3 py-2 rounded-lg bg-slate-900/60 border border-slate-700/40 text-slate-200 text-sm focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/20"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                        ...
                        <select v-model="createForm.stage" class="w-full px-3 py-2 rounded-lg bg-slate-900/60 border border-slate-700/40 text-slate-200 text-sm">
                            <option value="READY">READY</option>
                            <option value="DRAFT">DRAFT</option>
                            <option value="PROPOSED">PROPOSED</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select v-model="createForm.target" class="w-full px-3 py-2 rounded-lg bg-slate-900/60 border border-slate-700/40 text-slate-200 text-sm">
                            <option value="auto">auto</option>
                            <option value="chatgpt">chatgpt</option>
                            <option value="gemini">gemini</option>
                            <option value="claude">claude</option>
                    ...
                    <div>
                        ...
                        <input v-model.number="createForm.priority" type="number" min="0" max="10"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <select v-model="createForm.mission_id"
                                class="w-full px-3 py-2 rounded-lg bg-slate-900/60 border border-slate-700/40 text-slate-200 text-sm">
        ...
        <Modal :open="showReasonModal" @update:open="showReasonModal = $event" size="sm">
            <template #title>Motivo operacional</template>
            ...
            <div class="space-y-3">
                <textarea
                    v-model="reasonInput" rows="3" autofocus
                    class="w-full px-3 py-2 rounded-lg bg-slate-900/60 border border-slate-700/40 text-slate-200 text-sm focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/20"
            ...
            <template #footer>
                <div class="flex justify-end gap-2 w-full">
                    ...
                    <Button variant="primary" size="sm" @click="confirmReasonAndExecute" :disabled="!reasonInput.trim()">
