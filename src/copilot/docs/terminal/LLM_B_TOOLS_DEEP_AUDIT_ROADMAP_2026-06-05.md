...
- [ ] Próximo gap UX: telas antigas de history, catalog search/freshness/conflicts e provider detail residual ainda precisam de auditoria de ANSI manual por ondas com testes de ausência de `\x1b[`.
- [ ] Próximo passo: auditar `catalog refresh` live/progress e `selection explain/search` para priorizar as superfícies pré-build com maior volume de logs/progresso e eliminar ANSI manual por ondas, sem reescrever todo o `/byok` legado de uma vez.
