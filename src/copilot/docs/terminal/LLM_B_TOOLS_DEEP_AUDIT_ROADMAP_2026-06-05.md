...
- [ ] Próximo gap UX: `BYOK model route` e uma linha residual de listagem/recommend ainda usam ANSI manual; migrar com teste de ausência de `\x1b[` e manter rastreabilidade de decisão de rota.
- [ ] Próximo passo: auditar `selection explain/search`, `model route` live/progress e listagens de modelo para finalizar o arquivo `/byok` antes de voltar aos testes live LLM-B.
