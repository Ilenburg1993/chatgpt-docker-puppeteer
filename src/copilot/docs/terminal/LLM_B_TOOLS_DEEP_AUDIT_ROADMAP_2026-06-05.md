...
- [ ] Próximo gap UX: `/byok gateway commands` ainda exibe descrições canônicas em inglês e códigos de preset em excesso; decidir se o inventário terminal deve ter tradução humana separada da descrição técnica do package.
- [ ] Próximo gap UX: telas antigas de eligibility/history ainda usam ANSI manual fora do tema; migrar por onda depois das superfícies BYOK auto/operator-ready.
- [ ] Próximo passo: auditar `catalog refresh` live/progress e `selection audit` para priorizar as superfícies pré-build com maior volume de logs/progresso e eliminar ANSI manual por ondas, sem reescrever todo o `/byok` legado de uma vez.
