...
- [ ] Próximo gap UX: telas antigas de history, SQLite mirror, OpenAI schema e provider explain ainda usam ANSI manual fora do tema; migrar por ondas com testes de ausência de `\x1b[`.
- [ ] Próximo passo: auditar `catalog refresh` live/progress, `selection explain/search` e espelhos SQLite para priorizar as superfícies pré-build com maior volume de logs/progresso e eliminar ANSI manual por ondas, sem reescrever todo o `/byok` legado de uma vez.
