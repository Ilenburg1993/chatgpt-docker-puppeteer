...
- [x] 2026-06-05: títulos públicos BYOK/model-gateway foram re-humanizados em massa: rota de modelo, requisitos de ambiente, gate pré-K, prontidão pré-build, comandos canônicos, operador pronto, refresh/diff/elegibilidade/catálogo, automação, standby, política, diagnóstico, probes, perfis/modelos/recomendação e fixtures deixaram de exibir cabeçalhos crus como `BYOK model-gateway auto`, `Provider`, `Policy`, `Export`, `Auto apply`, `Recoveries` e `nao`.
- [x] 2026-06-05: `session.model_changed` agora narra confirmação SDK casada com pedido vivo BYOK como `Modelo confirmado`, mantendo reconfirmação redundante silenciosa e preservando o envelope SSE com `matchedTerminalRequest`.
- [x] 2026-06-05: live PTY `diagnostic-ux-cycle` PASS em `artifacts/terminal-live/terminal-ux-byok-humanized-20260605-1800`; a revisão humana do plain log revelou resíduos menores (`Export` em `/scope context`, `limite numerico`, `aggregator auto`, `wire -`) agora cobertos por testes focados.
- [x] 2026-06-05: `/tools all` deixou de reexpor `report_intent`/`report_intent_local` nos detalhes de lifecycle; a superfície humana mostra `Intenção capturada` e reserva nomes crus para `/tools raw` e `/events --raw`.
- [x] 2026-06-05: rótulos públicos de troca automática passaram de `Auto`/`Auto mode` para `Modo automático` em `/model auto` e no evento SDK de auto mode switch.
