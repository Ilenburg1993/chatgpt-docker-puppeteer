...
A investigação desta etapa foi deliberadamente separada de LLM-B e partiu das métricas reais do próprio MCP durante o trabalho no repositório. O baseline do processo anterior tinha `104` chamadas sem erro. Entre as tools ordinárias de maior uso, `repo_search_text` aparecia com **25 chamadas / 271 ms médios** (~`6,8 s` de custo acumulado de handler), `repo_read_file` com **25 / 50 ms** (~`1,25 s`) e `repo_apply_patch` com **13 / 39 ms** (~`0,51 s`). Mais importante: apenas read/search/patch somavam **63 round-trips**. Isso mostrou que a latência percebida não vinha apenas de handlers isoladamente lentos; vinha também da multiplicação de chamadas host → MCP → handler → host durante uma investigação ou edição longa.

A frente foi corrigida em cinco camadas complementares:

- [x] `mcp_latency_dashboard` deixa de ordenar apenas por média e passa a expor **`highestCumulativeCost`** e **`highestCallPressure`**, com `totalDurationMs`, participação nas chamadas e participação no tempo. Assim, uma tool moderadamente rápida mas chamada dezenas de vezes deixa de ficar invisível atrás de operações de rede raras;
- [x] `repo_read_file` e `repo_search_text` ganharam modo `batch` limitado a `10` itens e concorrência interna limitada a `4`, sem criar novas tools de topo. Isso reduz round-trips sem reduzir a superfície funcional de `116` tools;
- [x] o batch de read/search evita duplicar todos os corpos/resultados no legado `content`: o payload pesado fica uma vez em `structuredContent.results` e `content` carrega apenas um resumo curto. O modo single continua compatível;
- [x] `repo_search_text` ganhou fast path de **substring literal exata no SQLite** (`sqlite-index-literal`) sobre os chunks crus, incluindo busca scoped e case-sensitive. Para case-insensitive o fast path fica limitado a queries ASCII, evitando fingir case-fold Unicode que o `lower()` do SQLite não fornece. Em miss, FTS5/`rg` continuam como fallback;
- [x] `repo_apply_patch_batch` passou a aceitar múltiplas operações no **mesmo path**. As operações são avaliadas sequencialmente contra o conteúdo virtual produzido pela anterior e, por arquivo, usam um único `lock → read → patch N → write → cache invalidation`. A escrita do grupo é atômica por arquivo; batches com arquivos distintos preservam o comportamento de falha parcial já existente;
- [x] `repo_patch_batch_plan` agora aponta para **uma única chamada** `repo_apply_patch_batch`, em vez de reconstruir `N` chamadas `repo_apply_patch` após o planejamento;
- [x] `mcp_session_profile`/`IO_GUIDANCE` passa a recomendar explicitamente batching de read/search, batch de patches no mesmo arquivo e fixed-string search com `contextLines=0` quando suficiente, para converter a capacidade nova em comportamento operacional recorrente;
- [x] a metadata repetida em `tools/list` foi compactada: o status `invoking` continua humano/específico, enquanto o status `invoked` passa ao compacto `OK`. Nenhuma tool, schema de segurança ou capacidade foi removida.

A prova objetiva pós-reload mostrou o comportamento esperado. A implementação literal foi ainda refinada para não devolver apenas o range/snippet do chunk: o SQLite seleciona os chunks candidatos e o formatter local recompõe **linhas exatas no contrato `path:line:text`**, preservando a semântica de grep sem subprocesso. No reload final, duas buscas literais consecutivas por `patchTextBatchLocked(` em `src/copilot` retornaram `engine=sqlite-index-literal`, linhas exatas e ficaram em **92 ms médios / 36 ms na última**; a última chamada é aproximadamente **7,5× mais rápida** que a média histórica de `271 ms` desse hot tool. Duas leituras repetidas do mesmo window após reload ficaram em **11 ms médios / 7 ms na última**, contra `50 ms` médios no baseline anterior. Um dry-run live com **duas operações no mesmo arquivo** via `repo_apply_patch_batch` foi aceito como `groupedSameFile=true` e completou em **11 ms de handler**, comprovando que a antiga restrição de paths únicos deixou de forçar dois ciclos separados.

A compactação de metadata compensou inclusive o crescimento dos schemas de batch. Logo após adicionar os campos de batch, o smoke autenticado mediu `tools/list = 130.406 bytes`, perigosamente próximo do teto de `131.072`. Depois da compactação e novo reload, o smoke ficou totalmente verde com **116/116 tools**, nenhum missing/unexpected, `tools/list = 127.971 bytes` e aproximadamente **3.101 bytes de headroom**. Isso é `2.435 bytes` menor que o estado transitório pós-batch e `1.947 bytes` menor que o baseline anterior de `129.918`, apesar da superfície ter ficado mais capaz. No mesmo smoke, o fluxo OAuth autenticado concluiu em **965 ms** e a orquestração total em **980 ms**.

A validação causal permaneceu focada: `test_mcp_tools.spec.js` passou após os novos modos de batch e o patch atômico no mesmo arquivo; `test_io_index_sqlite.spec.js` passou com casos de substring literal, case sensitivity e path scoped; `test_io_search.spec.js` passou cobrindo a expansão do chunk literal para linhas `path:line:text`; `test_mcp_registry.spec.js` passou com a metadata compactada. O gate final, já após o refinamento semântico da busca literal, fechou com **strict typecheck verde em `15,855 s`** e **lint verde em `18,542 s`**. Runs intermediários de typecheck/lint encontraram problemas locais reais introduzidos nesta etapa (enum de operação, `exactOptionalPropertyTypes`, posição de JSDoc e regra `no-control-regex`), todos corrigidos antes do gate final.

A conclusão operacional muda a prioridade de otimização: com read quente em poucos milissegundos, search literal quente em dezenas de milissegundos e patch batch em um único ciclo, o maior ganho futuro tende a vir de **reduzir a quantidade de chamadas** e o volume de payload por tarefa, não de micro-otimizar o filesystem. Uma otimização adicional possível é eliminar a dupla resolução/policy check de path entre o adapter MCP e o `workspace-io`, mas isso deve ser feito apenas com uma capability interna opaca/validada; um simples `trustPath=true` aumentaria velocidade às custas da fronteira de segurança e, portanto, não foi introduzido nesta etapa.
