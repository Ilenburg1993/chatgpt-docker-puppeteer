...
/** @param {NodeJS.ProcessEnv} env @returns {CloudflaredVersion} */
export function readCloudflaredVersion(env) {
    if (!env) throw new TypeError('cloudflared version probe requires explicit env.');
...
export function createCloudflareManagedProcessController(config) {
    ...
    const stopTimeoutMs = normalizeStopTimeoutMs(config.processStopTimeoutMs);
...
async function ensureBoundDetachedProcess(paths, io, options) {
    ...
    let child;
    try {
        if (!options.env) throw new TypeError(`Detached process ${options.name} requires an explicit environment.`);
...
function normalizeStopTimeoutMs(value) {
    return Number.isFinite(value) && Number(value) >= 500 ? Math.floor(Number(value)) : DEFAULT_STOP_TIMEOUT_MS;
}
