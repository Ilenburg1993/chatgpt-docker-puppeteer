...
/**
 * @param {NodeJS.ProcessEnv | import('../environment-authority.js').CloudflareEnvironmentAuthority | { authority?: import('../environment-authority.js').CloudflareEnvironmentAuthority; env?: NodeJS.ProcessEnv }} input
 * @returns {Promise<CloudflareRemoteApiConfig>}
 */
export async function readCloudflareRemoteApiConfig(input) {
    const merged = await resolveCloudflareRemoteApiEnvironment(input);
    const tunnelConfig = readCloudflareTunnelConfig(merged);
    return {
        apiToken:
        ...
        credentialSources: buildCredentialSources(merged),
    };
}

/**
 * @param {{ authority?: import('../environment-authority.js').CloudflareEnvironmentAuthority; env?: NodeJS.ProcessEnv; cacheTtlMs?: number; forceRefresh?: boolean }} [options]
 * @returns {Promise<Record<string, unknown> & { ok: boolean }>}
 */
export async function auditCloudflareRemoteTunnel(options = {}) {
    const apiConfig = await readCloudflareRemoteApiConfig(options);
...
function buildCredentialSources(env) {
    ...
    for (const key of [
        ...
        if (String(env[key] ?? '').trim()) sources.push(`generation:${key}`);
    }
    return sources;
}
