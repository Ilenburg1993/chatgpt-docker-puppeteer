// @ts-check
/** Cloudflare Tunnel transport benchmark planning domain. */

import { readCloudflaredMetricsSnapshot } from '#copilot/mcp/public/cloudflare/observability';
import { readTransportBenchmarkState, TRANSPORT_BENCHMARK_STATE_PATH } from './state.js';
 ...
 */
export async function buildCloudflareTransportBenchmarkPlan(input, config) {
    if (!config) throw new TypeError('Cloudflare transport benchmark plan requires a config projection.');
    const metrics = input.includeMetricsBaseline === true ? await safeMetricsSnapshot(input.timeoutMs, config) : null;
 ...
 * @returns {Promise<Record<string, unknown>>}
 */
async function safeMetricsSnapshot(timeoutMs, config) {
    try {
        return await readCloudflaredMetricsSnapshot(timeoutMs === undefined ? {} : { timeoutMs }, config);
    } catch (error) {
        return {
            ok: false,
