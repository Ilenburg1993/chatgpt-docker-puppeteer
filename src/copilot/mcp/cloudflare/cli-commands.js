...
import { runCanonicalConnectorSmoke } from './connector-smoke.js';
...
const COMMANDS = [
    ...
    [
        ...
        () => writeJsonAndSetExit(auditCloudflareRemoteTunnel()),
    ],
    [
        'config-audit',
        'Audit local Cloudflare MCP configuration posture.',
        () => writeJsonAndSetExit(auditCloudflareConfigPosture()),
    ],
    [
        'plan-capabilities-audit',
        'Audit Cloudflare plan capabilities.',
        () => writeJsonAndSetExit(auditCloudflarePlanCapabilities()),
    ],
    ...
    [
        ...
        () => writeJsonAndSetExit(buildCloudflareMcpPassthroughPlan()),
    ],
    [
        'mcp-passthrough-diff',
        'Diff MCP passthrough rules.',
        () => writeJsonAndSetExit(diffCloudflareMcpPassthroughPlan()),
    ],
...
export async function runCloudflareCli(argv = process.argv, env = process.env) {
    ...
    if (!command) {
        await writeJsonAndSetExit({
            ...
            availableCommands: [...COMMAND_MAP.keys()].sort(),
        });
        return;
    }
...
/** @param {CloudflareCliContext} context */
async function runOAuthSmoke({ env }) {
    await writeJsonAndSetExit(
        await runCloudflareSmoke({
            config: readCloudflareTunnelConfig(env),
            authenticated: true,
            env,
...
async function runUp({ env }) {
    await writeJsonAndSetExit(
        await startManagedStack({ config: readCloudflareTunnelConfig(env), env, restart: false }),
    );
}
/** @param {CloudflareCliContext} context */
async function runDown({ env }) {
    await writeJsonAndSetExit(await stopManagedStack(readCloudflareTunnelConfig(env)));
}
/** @param {CloudflareCliContext} context */
async function runRestart({ env }) {
...
/** @param {CloudflareCliContext} context */
async function runOriginApply({ env }) {
    await writeJsonAndSetExit(
...
/** @param {CloudflareCliContext} context */
async function runEdgePolicyApply({ argv }) {
    await writeJsonAndSetExit(
