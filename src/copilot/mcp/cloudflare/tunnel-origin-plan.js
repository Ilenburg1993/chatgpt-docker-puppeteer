 ...
 *
 * @param {{ env?: NodeJS.ProcessEnv; dryRun?: boolean; confirmApply?: boolean }} [options]
 * @returns {Promise<Record<string, unknown> & { ok: boolean }>}
 */
export async function applyCloudflareTunnelOriginPlan(options = {}) {
    ...
    const localConfig = await readCloudflareRemoteApiConfig(env);
    const desired = buildDesiredTunnelOriginConfig(localConfig);
    const preflight = await buildCloudflareTunnelOriginPlan({ env });
    ...
    const reference = await resolveCloudflareRemoteTunnelReference({ env });
...
const ORIGIN_REQUEST_DURATION_KEYS = new Set(['connectTimeout', 'keepAliveTimeout', 'tcpKeepAlive', 'tlsTimeout']);
...
function normalizeOriginRequestForApi(originRequest) {
    ...
    for (const [key, value] of Object.entries(originRequest)) {
        normalized[key] = ORIGIN_REQUEST_DURATION_KEYS.has(key) ? parseCloudflareDurationNanos(value, key) : value;
    }
    return normalized;
}
