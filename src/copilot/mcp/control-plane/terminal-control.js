 ...
 * MCP process. The caller may choose any executable, shell command, cwd and environment that this OS identity can use.
 * The control plane itself does not maintain a command allowlist.
 *
 ...
 *
 * @module copilot/mcp/control-plane/terminal-control
 */
...
export const MCP_TERMINAL_CONTROL_VERSION = 1;
...
export async function executeTerminalCommand(input) {
    const spec = normalizeTerminalCommandSpec(input);
    ...
    return await new Promise((resolve) => {
        ...
        if (spec.timeoutMs > 0) {
            timeout = setTimeout(() => {
                ...
                setTimeout(() => killProcessTree(child.pid, 'SIGKILL'), 1_500).unref();
            }, spec.timeoutMs);
            timeout.unref();
        }
        ...
        const finish = (exitCode, signal, error = null) => {
            ...
            resolve({
                success: error === null && exitCode === 0 && !timedOut,
 ...
 */
export async function executeTerminalCommandBatch(commands, options = {}) {
    ...
    const finalized = results.map(
        (result, index) => result ?? { index, success: false, skipped: true, reason: 'fail-fast-aborted' },
    );
...
export async function openTerminalSession(input = {}) {
    ...
    const spec = normalizeTerminalSessionSpec(input);
...
export async function controlTerminalSession(input) {
    ...
    if (action === 'signal') {
        ...
        else if (record.pid) killProcessTree(record.pid, signal, input.processGroup !== false);
    ...
    if (action === 'close') {
        ...
        else if (record.pid) killProcessTree(record.pid, 'SIGTERM', input.processGroup !== false);
        if (graceMs > 0) await waitForSessionExit(record, graceMs);
        if (record.status === 'running') {
            if (record.backend === 'pty' && record.pty) record.pty.kill('SIGKILL');
            else if (record.pid) killProcessTree(record.pid, 'SIGKILL', input.processGroup !== false);
...
/** @param {TerminalCommandSpec} input */
function normalizeTerminalCommandSpec(input) {
    ...
    return {
        ...
        env: buildTerminalEnvironment(input.env, input.inheritEnv !== false),
...
function normalizeTerminalSessionSpec(input) {
    const shell = input?.shell !== false;
...
function resolveTerminalCwd(cwd) {
    if (cwd === undefined || cwd === '') return getMcpWorkspaceRoot();
    return path.isAbsolute(cwd) ? path.normalize(cwd) : path.resolve(getMcpWorkspaceRoot(), cwd);
}

/** @param {Record<string, string | null> | undefined} override @param {boolean} inherit */
function buildTerminalEnvironment(override, inherit) {
...
/** @param {number | undefined} pid @param {NodeJS.Signals} signal @param {boolean} [processGroup] */
function killProcessTree(pid, signal, processGroup = true) {
    if (!pid) return false;
    try {
        process.kill(process.platform !== 'win32' && processGroup ? -pid : pid, signal);
        return true;
    } catch {
        try {
