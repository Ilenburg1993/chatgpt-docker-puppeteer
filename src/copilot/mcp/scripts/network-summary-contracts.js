 ...
 *
 * Zero external dependencies by design. The parser accepts JSONC comments and
 * trailing commas while rejecting duplicate object keys before object materialization.
 *
...
const SECRET_BEARING_KEY = /(?:^|[_-])(?:api[_-]?key|authorization|bearer|cookie|password|private[_-]?key|secret|token)(?:$|[_-])/iu;
...
/**
 * @param {string} source
 * @returns {{ value: unknown; duplicateKeys: Array<{ path: string; key: string; offset: number }> }}
 */
export function parseJsoncWithDuplicateKeys(source) {
    ...
    let cursor = 0;
    /** @type {Array<{ path: string; key: string; offset: number }>} */
    const duplicateKeys = [];
    ...
    const parseValue = (path) => {
        ...
        if (token.type === 'punct' && token.value === '{') {
            ...
            while (true) {
                ...
                if (keyToken?.type !== 'string') throw new SyntaxError(`Expected object key at offset ${keyToken?.offset ?? clean.length}.`);
                cursor += 1;
    ...
    if (current()?.type !== 'eof') throw new SyntaxError(`Unexpected trailing JSON token at offset ${current()?.offset ?? clean.length}.`);
...
function stringArray(value) {
    return Array.isArray(value) && value.every((item) => typeof item === 'string') ? /** @type {string[]} */ (value) : null;
}
...
async function isExistingWorkspaceFile(workspacePath) {
    ...
    try {
        const info = await stat(resolve(REPO_ROOT, workspacePath));
        return info.isFile();
    } catch {
        return false;
    }
 ...
 * @param {{ duplicateKeys?: Array<{ path: string; key: string; offset: number }> }} [options]
 */
export async function validateNetworkSummaryContractsValue(parsed, options = {}) {
    ...
    if (typeof root['schemaVersion'] !== 'string' || !SEMVER.test(root['schemaVersion'])) errors.push('schemaVersion must be semantic version x.y.z.');
    if (root['status'] !== 'canonical') warnings.push(`catalog status is '${String(root['status'] ?? 'missing')}', expected 'canonical'.`);
    if (root['canonicalPath'] !== DEFAULT_NETWORK_SUMMARY_CONTRACTS_FILE) errors.push(`canonicalPath must be ${DEFAULT_NETWORK_SUMMARY_CONTRACTS_FILE}.`);

    const common = asRecord(root['common']);
    const formats = asRecord(common?.['fileFormats']);
    if (!formats || Object.keys(formats).length === 0) errors.push('common.fileFormats must declare at least one format.');
    const artifacts = asRecord(root['artifacts']);
    if (!artifacts || Object.keys(artifacts).length === 0) errors.push('artifacts must declare at least one artifact contract.');
    ...
    if (artifacts) {
        for (const [artifactId, candidate] of Object.entries(artifacts)) {
            ...
            if (!artifactPath || !isAbsolute(artifactPath)) errors.push(`artifact ${artifactId} path must be absolute.`);
            else if (seenPaths.has(artifactPath)) errors.push(`artifact path ${artifactPath} is reused by ${String(seenPaths.get(artifactPath))} and ${artifactId}.`);
            else seenPaths.set(artifactPath, artifactId);

            const format = typeof artifact['format'] === 'string' ? artifact['format'] : '';
            if (!format || !formats?.[format]) errors.push(`artifact ${artifactId} references unknown format '${format || 'missing'}'.`);
            ...
            if (!producer) errors.push(`artifact ${artifactId} producer is missing.`);
                ...
                if (!(await isExistingWorkspaceFile(producer))) errors.push(`artifact ${artifactId} producer does not exist: ${producer}.`);
            }
            const producerVersion = typeof artifact['producerVersion'] === 'string' ? artifact['producerVersion'] : '';
            if (!PRODUCER_VERSION.test(producerVersion)) errors.push(`artifact ${artifactId} producerVersion must be vX.Y.Z.`);
            ...
            if (!requiredKeys) errors.push(`artifact ${artifactId} requiredKeys must be a string array.`);
                ...
                for (const duplicate of duplicates(requiredKeys)) errors.push(`artifact ${artifactId} duplicates requiredKey '${duplicate}'.`);
            ...
            for (const field of ['acceptedStatusValues', 'invariants']) {
                ...
                else for (const duplicate of duplicates(values)) errors.push(`artifact ${artifactId} duplicates ${field} value '${duplicate}'.`);
            }
        }
    }
    ...
    if (!validatorProfile) errors.push('validatorProfile must be an object.');
        ...
        if (validatorProfile['implementationStatus'] !== 'active') errors.push("validatorProfile.implementationStatus must be 'active'.");
        if (validatorProfile['recommendedCommand'] !== DEFAULT_COMMAND) errors.push(`validatorProfile.recommendedCommand must be '${DEFAULT_COMMAND}'.`);
        if (validatorProfile['explainerCommand'] !== DEFAULT_EXPLAIN_COMMAND) errors.push(`validatorProfile.explainerCommand must be '${DEFAULT_EXPLAIN_COMMAND}'.`);
        const checks = stringArray(validatorProfile['requiredChecks']);
        if (!checks || checks.length === 0) errors.push('validatorProfile.requiredChecks must be a non-empty string array.');
        else for (const duplicate of duplicates(checks)) errors.push(`validatorProfile.requiredChecks duplicates '${duplicate}'.`);
    }
...
export async function validateNetworkSummaryContractsFile(workspacePath = DEFAULT_NETWORK_SUMMARY_CONTRACTS_FILE) {
    ...
    const validation = await validateNetworkSummaryContractsValue(parsed.value, { duplicateKeys: parsed.duplicateKeys });
...
async function main() {
    ...
    if (!['validate', 'explain'].includes(action)) {
        process.stderr.write(`Usage: node src/copilot/mcp/scripts/network-summary-contracts.js <validate|explain> [catalog]\n`);
        process.exitCode = 2;
        return;
    }
