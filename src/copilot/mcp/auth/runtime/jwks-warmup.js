 ...
 *
 * @module copilot/mcp/auth/runtime/jwks-warmup
 */

import { logMcp } from '#copilot/mcp/public/observability';
import { warmMcpRemoteJwks } from '../resource-server/service.js';
...
/**
 * Cancel delayed JWKS warmup and await the active generation. A stale remote fetch may finish physically, but cannot
 * publish state or log itself as the current runtime after stop resolves.
 *
 * @returns {Promise<void>}
 */
export async function stopMcpAuthJwksWarmup() {
    warmupGeneration += 1;
    clearWarmupTimer();
    warmupState = { ...warmupState, scheduled: false };
    const activeRun = warmupRunPromise;
    if (activeRun) await activeRun.catch(() => undefined);
    warmupState = { ...warmupState, scheduled: false, running: false };
}
