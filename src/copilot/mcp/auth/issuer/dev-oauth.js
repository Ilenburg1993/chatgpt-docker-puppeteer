...
/**
 * Create one isolated built-in OAuth issuer generation. All authorization codes, PAR requests, client metadata,
 * persistence coordination, key material, request budgets and replay caches live in this lexical runtime rather than at
 * module scope. Multiple generations may coexist safely during tests, reloads or overlapping listener teardown.
 *
 * @param {{
 *   processConfig?: import('./config.js').DevOAuthProcessConfig | NodeJS.ProcessEnv;
 *   replay?: Pick<ReturnType<typeof import('../persistence/replay-store.js').createOAuthReplayCapability>, 'remember' | 'status'>;
 *   compatibilityObserver?: (observation: Record<string, unknown>) => void | Promise<void>;
 * }} [options]
 */
export function createDevOAuthRuntime(options = {}) {
    const boundProcessConfig = resolveDevOAuthProcessConfig(options.processConfig);
    const replay =
        options.replay ??
        Object.freeze({
            remember: () => ({
    ...
    const rememberPersistentOAuthReplay = (
        /** @type {string} */ namespace,
        /** @type {string} */ replayKey,
        /** @type {number} */ expiresAtMs,
    ) => replay.remember(namespace, replayKey, expiresAtMs);
    const readPersistentOAuthReplayStatus = () => replay.status();
    const compatibilityObserver =
        typeof options.compatibilityObserver === 'function' ? options.compatibilityObserver : null;
    const PRIVATE_NETWORK_BLOCK_LIST = createPrivateNetworkBlockList();

    /**
     * Emit only the already-bounded compatibility projection supplied by this issuer. Observer failures are isolated
     * from OAuth behavior and the promise is intentionally not awaited on the request path.
     *
     * @param {Record<string, unknown>} observation
     */
    function recordCompatibilityObservation(observation) {
        if (!compatibilityObserver) return;
        try {
            const pending = compatibilityObserver(observation);
            if (pending && typeof pending === 'object' && 'catch' in pending && typeof pending.catch === 'function') {
                void pending.catch(() => undefined);
            }
        } catch {
            // Compatibility telemetry must never alter OAuth control flow.
        }
    }
     ...
     * @param {import('./config.js').DevOAuthProcessConfig | NodeJS.ProcessEnv} [processConfig]
     * @returns {boolean}
     */
    function isBuiltInDevOAuthEnabled(config, processConfig = boundProcessConfig) {
        const issuerConfig = resolveDevOAuthProcessConfig(processConfig);
        if (!issuerConfig.enabled) return false;
        if (config.mode !== 'oauth' || config.expectedIssuer !== config.resource) return false;
        return isAllowedDevOAuthResourceIdentifier(config.resource, issuerConfig);
    }
     ...
     * @returns {boolean}
     */
    function isAllowedDevOAuthResourceIdentifier(resource, issuerConfig) {
        try {
            ...
            if (url.protocol === 'http:' && isLoopbackRedirectHostname(url.hostname)) {
                return issuerConfig.allowHttpLocalhost;
            }
            return false;
        } catch {
            return false;
     ...
     * @returns {boolean}
     */
    function isDevOAuthCimdEnabled(issuerConfig) {
        return issuerConfig.cimdEnabled;
    }
     ...
     * @returns {Record<string, unknown>}
     */
    function buildBuiltInDevOpenIdConfiguration(config, issuerConfig) {
        return {
            ...buildBuiltInDevOAuthMetadata(config, issuerConfig),
     ...
     * @returns {Record<string, unknown>}
     */
    function buildBuiltInDevOAuthClientMetadata(config) {
     ...
     */
    async function handleBuiltInDevOAuthRequest(req, res, url, config, processConfig = boundProcessConfig) {
        const issuerConfig = resolveDevOAuthProcessConfig(processConfig);
        bindDevOAuthResponseConfig(res, issuerConfig);
        if (!isBuiltInDevOAuthEnabled(config, issuerConfig)) return false;
        ...
        if (req.method === 'GET' && isAuthorizationServerDiscoveryEndpoint(url.pathname, config)) {
            writeJson(
                ...
                isOpenIdDiscoveryEndpoint(url.pathname)
                    ? buildBuiltInDevOpenIdConfiguration(config, issuerConfig)
                    : buildBuiltInDevOAuthMetadata(config, issuerConfig),
            );
            return true;
        }
        ...
        if (req.method === 'GET' && url.pathname === '/oauth/jwks.json') {
            const { publicJwks } = await getKeyMaterial(issuerConfig);
            writeJson(res, 200, { keys: publicJwks });
            return true;
        }

        if (req.method === 'GET' && url.pathname === '/oauth/status') {
            if (!isDevOAuthDiagnosticsEnabled(issuerConfig)) return false;
            writeJson(res, 200, await buildDevOAuthStatus(config, issuerConfig));
            return true;
        }

        if (req.method === 'POST' && url.pathname === '/oauth/register') {
            await ensureRegisteredClientsLoaded(issuerConfig);
            ...
            const { clientTtlSeconds } = readDevOAuthClientLifetimePolicy(issuerConfig);
            ...
            await persistRegisteredClients(issuerConfig);
            recordCompatibilityObservation({
                kind: 'oauth-client',
                clientSource: 'dcr',
                hostClass: 'unknown',
            ...
            writeJson(res, 201, {
                ...
                client_id_expires_at: Math.floor(
                    (client.expiresAt ?? client.createdAt + clientTtlSeconds * 1000) / 1000,
                ),
        ...
        if (req.method === 'POST' && url.pathname === '/oauth/par') {
            await handlePushedAuthorizationRequest(req, res, config, issuerConfig);
            return true;
        }

        if (req.method === 'GET' && url.pathname === '/oauth/authorize') {
            await handleAuthorize(res, url, config, issuerConfig);
            return true;
        }

        if (req.method === 'POST' && url.pathname === '/oauth/token') {
            await handleToken(req, res, config, issuerConfig);
            return true;
        }

        if (req.method === 'POST' && url.pathname === '/oauth/revoke') {
            await handleRevoke(req, res, config, issuerConfig);
            return true;
        }

        if (req.method === 'POST' && url.pathname === '/oauth/introspect') {
            await handleIntrospect(req, res, config, issuerConfig);
            return true;
        }

        if (req.method === 'GET' && url.pathname === '/oauth/userinfo') {
            await handleUserInfo(req, res, config, issuerConfig);
            return true;
        }
    ...
    async function buildDevOAuthStatus(config, issuerConfig) {
        ...
        return {
            ...
            cimdEnabled: isDevOAuthCimdEnabled(issuerConfig),
            trustedChatGptCimdFallbackEnabled: isTrustedChatGptCimdFallbackEnabled(issuerConfig),
            trustedChatGptCimdFastPathEnabled: isTrustedChatGptCimdFastPathEnabled(issuerConfig),
            trustedClaudeCimdFallbackEnabled: isTrustedClaudeCimdFallbackEnabled(issuerConfig),
            resourceParameterRequired: isResourceParameterRequired(issuerConfig),
            dpopEnabled: isDevOAuthDpopEnabled(issuerConfig),
            dpopAuthorizationCodeBindingEnabled: true,
            dpopNonceRequired: isDpopNonceRequired(issuerConfig),
            dpopTypRequired: isDpopTypRequired(issuerConfig),
            introspectionEnabled: true,
            introspectionClientAuthenticationRequired: isIntrospectionClientAuthenticationRequired(issuerConfig),
    ...
    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    function isDevOAuthDiagnosticsEnabled(issuerConfig) {
        return issuerConfig.diagnosticsEnabled;
    }

    /**
     * MCP 2025-11-25 requires clients to send a Resource Indicator in authorization and token requests.
     * @param {import('./config.js').DevOAuthProcessConfig} issuerConfig
     */
    function isResourceParameterRequired(issuerConfig) {
        return issuerConfig.resourceParameterRequired;
    }

    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    function isDevOAuthDpopEnabled(issuerConfig) {
        return issuerConfig.dpop.enabled;
    }

    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    function isDpopNonceRequired(issuerConfig) {
        return issuerConfig.dpop.nonceRequired;
    }

    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    function isDpopTypRequired(issuerConfig) {
        return issuerConfig.dpop.typRequired;
    }

    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    function isIntrospectionClientAuthenticationRequired(issuerConfig) {
        return issuerConfig.introspectionClientAuthenticationRequired;
    }
     ...
     * }>}
     */
    async function getKeyMaterial(issuerConfig) {
        const configKey = [
            issuerConfig.storageConfigKey,
            issuerConfig.signing.algorithm,
            String(issuerConfig.signing.includeLegacyRs256),
            String(issuerConfig.signing.keyRotationRequested),
        ].join('\n');
        if (!keyMaterialPromise || keyMaterialConfigKey !== configKey) {
            keyMaterialConfigKey = configKey;
            keyMaterialPromise = (async () => {
                const alg = readDevOAuthSigningAlgorithm(issuerConfig);
                const privateKey = await readOrCreatePrivateKey(alg, issuerConfig);
                const publicJwk = await exportPublicJwk(privateKey);
                const kid = await calculateJwkThumbprint(publicJwk);
                const activePublicJwk = { ...publicJwk, kid, alg, use: 'sig' };
                const publicJwks = [activePublicJwk];
                let legacyKeyCount = 0;
                if (alg !== 'RS256' || isLegacyRsaJwksOverlapEnabled(issuerConfig)) {
                    const legacyPrivateKey = await readOrCreatePrivateKey('RS256', issuerConfig);
                    const legacyPublicJwk = await exportPublicJwk(legacyPrivateKey);
                    const legacyKid = await calculateJwkThumbprint(legacyPublicJwk);
                    if (legacyKid !== kid) {
                        publicJwks.push({ ...legacyPublicJwk, kid: legacyKid, alg: 'RS256', use: 'sig' });
                        legacyKeyCount += 1;
                    }
                }
     ...
     */
    async function readOrCreatePrivateKey(alg, issuerConfig) {
        const storage = getDevOAuthStorage(issuerConfig);
        if (!isDevOAuthKeyRotationRequested(issuerConfig)) {
            try {
     ...
     */
    async function persistPrivateKey(alg, privateKey, storage) {
        try {
    ...
    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    function readDevOAuthSigningAlgorithm(issuerConfig) {
        return issuerConfig.signing.algorithm;
    }

    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    function isLegacyRsaJwksOverlapEnabled(issuerConfig) {
        return issuerConfig.signing.includeLegacyRs256;
    }

    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    function isDevOAuthKeyRotationRequested(issuerConfig) {
        return issuerConfig.signing.keyRotationRequested;
    }
     ...
     * @returns {Promise<void>}
     */
    async function handlePushedAuthorizationRequest(req, res, config, issuerConfig) {
        ...
        const client = await resolveOAuthClientById(clientId, issuerConfig);
        if (!client) {
        ...
        if (isResourceParameterRequired(issuerConfig) && !resourceParam) errors.push('resource_missing');
    ...
    function consumePushedAuthorizationRequest(requestUri) {
        prunePushedAuthorizationRequests();
        if (!requestUri || requestUri.length > MAX_REQUEST_URI_LENGTH || hasControlCharacters(requestUri))
            return undefined;
     ...
     * @returns {Promise<void>}
     */
    async function handleAuthorize(res, url, config, issuerConfig) {
        ...
        if (!stateResult.ok || !nonceResult.ok) {
            writeJson(res, 400, {
                ...
                error_description: !stateResult.ok
                    ? 'state is invalid or too large.'
                    : 'nonce is invalid or too large.',
            });
            return;
        }
        ...
        if (client && !client.redirectUris.includes(redirectUri))
            authorizationRequestErrors.push('redirect_uri_mismatch');
        if (isResourceParameterRequired(issuerConfig) && !resourceParam)
            authorizationRequestErrors.push('resource_missing');
        ...
        if (authorizationCodes.size >= MAX_AUTHORIZATION_CODES) {
            rejectOrRedirectAuthorizeError(
                res,
                config,
                client,
                redirectUri,
     ...
     * @returns {Promise<void>}
     */
    async function handleToken(req, res, config, issuerConfig) {
        const body = await readOAuthRequestBody(req, res, { allowJson: false, allowForm: true });
        if (!body) return;
        const grantType = String(body['grant_type'] ?? '');
        const clientId = String(body['client_id'] ?? '');
        const compatibilityGrant = grantType === 'authorization_code' || grantType === REFRESH_TOKEN_GRANT;
        try {
            const dpop = await resolveDpopBindingForRequest(req, config, issuerConfig);
            if (!dpop.ok) {
            ...
            if (grantType === 'authorization_code') {
                await handleAuthorizationCodeToken(body, res, config, issuerConfig, dpop.jkt);
                return;
            }
            if (grantType === REFRESH_TOKEN_GRANT) {
                await handleRefreshToken(body, res, config, issuerConfig, dpop.jkt);
                return;
            }
            writeJson(res, 400, { error: 'unsupported_grant_type' });
        } finally {
            if (compatibilityGrant) {
     ...
     * @returns {Promise<void>}
     */
    async function handleAuthorizationCodeToken(body, res, config, issuerConfig, dpopJkt = '') {
     ...
     * @returns {Promise<void>}
     */
    async function handleRefreshToken(body, res, config, issuerConfig, dpopJkt = '') {
        const clientId = String(body['client_id'] ?? '');
        const credential = String(body[REFRESH_TOKEN_GRANT] ?? '');
        const resourceParam = String(body['resource'] ?? '');
        await ensureRenewCredentialsLoaded(issuerConfig);
     ...
     */
    async function resolveOAuthClientById(clientId, issuerConfig) {
        await ensureRegisteredClientsLoaded(issuerConfig);
        ...
        try {
            ...
            clientAssertionJwksCache.set(client.jwksUri, {
                ...
                expiresAt:
                    Date.now() +
                    Math.min(
                        fetched?.cacheTtlMs ?? CLIENT_ASSERTION_JWKS_CACHE_TTL_MS,
                        CLIENT_ASSERTION_JWKS_CACHE_TTL_MS,
                    ),
            });
     ...
     * @returns {Promise<void>}
     */
    async function handleRevoke(req, res, config, issuerConfig) {
        const body = await readOAuthRequestBody(req, res, { allowJson: false, allowForm: true });
        if (!body) return;
     ...
     * @returns {Promise<void>}
     */
    async function handleIntrospect(req, res, config, issuerConfig) {
        const body = await readOAuthRequestBody(req, res, { allowJson: false, allowForm: true });
        if (!body) return;
        ...
        const client = clientId ? await resolveOAuthClientById(clientId, issuerConfig) : undefined;
        if (isIntrospectionClientAuthenticationRequired(issuerConfig) && !client) {
            setBearerChallenge(res, config, 'invalid_client', 'Client authentication failed.');
            writeJson(res, 401, { error: 'invalid_client' });
            return;
        }
     ...
     * @returns {Promise<Record<string, unknown>>}
     */
    async function issueTokenSet(options, config, issuerConfig) {
        const { privateKey, kid, alg } = await getKeyMaterial(issuerConfig);
        const nowSeconds = Math.floor(Date.now() / 1000);
        const { accessTokenTtlSeconds, refreshTokenTtlSeconds } = readDevOAuthTokenLifetimePolicy(issuerConfig);
     ...
     * @param {import('./config.js').DevOAuthProcessConfig | NodeJS.ProcessEnv} [processConfig]
     */
    function readDevOAuthTokenLifetimePolicy(processConfig = boundProcessConfig) {
        const issuerConfig = resolveDevOAuthProcessConfig(processConfig);
        return {
            accessTokenTtlSeconds: issuerConfig.lifetimes.accessTokenTtlSeconds,
            refreshTokenTtlSeconds: issuerConfig.lifetimes.refreshTokenTtlSeconds,
            defaults: {
                accessTokenTtlSeconds: DEV_OAUTH_CONFIG_DEFAULTS.accessTokenTtlSeconds,
                refreshTokenTtlSeconds: DEV_OAUTH_CONFIG_DEFAULTS.refreshTokenTtlSeconds,
            },
        };
    }
    /**
     * @param {import('./config.js').DevOAuthProcessConfig | NodeJS.ProcessEnv} [processConfig]
     */
    function readDevOAuthClientLifetimePolicy(processConfig = boundProcessConfig) {
        const issuerConfig = resolveDevOAuthProcessConfig(processConfig);
        return {
            clientTtlSeconds: issuerConfig.lifetimes.clientTtlSeconds,
            defaults: { clientTtlSeconds: DEV_OAUTH_CONFIG_DEFAULTS.clientTtlSeconds },
        };
    }
     ...
     * @param {import('./config.js').DevOAuthProcessConfig | NodeJS.ProcessEnv} [processConfig]
     */
    function readDevOAuthPersistenceConfig(processConfig = boundProcessConfig) {
        return resolveDevOAuthProcessConfig(processConfig).persistence;
    }
    ...
    /**
     * Resolve all privileged OAuth state paths from one captured issuer configuration generation.
     * @param {import('./config.js').DevOAuthProcessConfig} issuerConfig
     * @returns {DevOAuthStorageBinding}
     */
    function resolveDevOAuthStorageBinding(issuerConfig) {
        return Object.freeze({
            storageConfigKey: issuerConfig.storageConfigKey,
            persistenceConfigKey: issuerConfig.persistenceConfigKey,
            keyFiles: issuerConfig.signing.keyFiles,
            refreshTokenFile: issuerConfig.persistence.refreshTokenFile,
            clientFile: issuerConfig.persistence.clientFile,
        });
    }
    ...
    function createDevOAuthStorage(binding) {
        const io = createConfiguredFsIo(
            createConfiguredFsGrant({
                id: 'mcp.auth.issuer.dev-oauth.storage',
                exactPaths: [
                    ...new Set([
     ...
     * @returns {DevOAuthStorage}
     */
    function getDevOAuthStorage(issuerConfig) {
        const binding = resolveDevOAuthStorageBinding(issuerConfig);
    ...
    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    function devOAuthPersistenceConfigKey(issuerConfig) {
        return issuerConfig.persistenceConfigKey;
    }
     ...
     * }>}
     */
    async function readDevOAuthPersistenceStatus(processConfig = boundProcessConfig) {
        const issuerConfig = resolveDevOAuthProcessConfig(processConfig);
        await ensureRenewCredentialsLoaded(issuerConfig);
        await ensureRegisteredClientsLoaded(issuerConfig);
        ...
        pruneConsumedRefreshTokenHashes();
        pruneRevokedRefreshTokenFamilies();
        if (pruned) await persistRenewCredentials(issuerConfig);
        const prunedClients = pruneRegisteredClients();
        if (prunedClients) await persistRegisteredClients(issuerConfig);
        const persistenceConfig = issuerConfig.persistence;
        return {
            ...persistenceConfig,
     ...
     * @returns {void}
     */
    function resetDevOAuthRuntimeForTests() {
        devOAuthStorage = null;
        keyMaterialPromise = null;
        keyMaterialConfigKey = null;
     ...
     * @param {import('./config.js').DevOAuthProcessConfig} issuerConfig
     */
    function deriveRegisteredClientExpiresAt(createdAt, issuerConfig) {
        return createdAt + issuerConfig.lifetimes.clientTtlSeconds * 1000;
    }
    ...
    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    async function ensureRenewCredentialsLoaded(issuerConfig) {
        const configKey = devOAuthPersistenceConfigKey(issuerConfig);
        ...
        renewCredentialsLoadPromise ??= loadRenewCredentials(issuerConfig);
        await renewCredentialsLoadPromise;
    }

    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    async function loadRenewCredentials(issuerConfig) {
        const storage = getDevOAuthStorage(issuerConfig);
        renewCredentials.clear();
        renewCredentialsLoaded = false;
        renewCredentialsLoadedFromFile = false;
        ...
        try {
            ...
            const code =
                error && typeof error === 'object' ? /** @type {{ code?: unknown }} */ (error).code : undefined;
            if (code !== 'ENOENT')
    ...
    function parseRevokedRefreshTokenFamilyRecords(parsed) {
        if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return [];
        const root = /** @type {Record<string, unknown>} */ (parsed);
        const rawRevoked = Array.isArray(root['revokedRefreshTokenFamilies'])
            ? root['revokedRefreshTokenFamilies']
            : [];
        const records = [];
    ...
    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    async function persistRenewCredentials(issuerConfig) {
        const storage = getDevOAuthStorage(issuerConfig);
        ...
        try {
            const body = {
                ...
                tokens: [...renewCredentials.entries()]
                    ...
                    .sort(
                        (left, right) =>
                            left.expiresAt - right.expiresAt || left.tokenHash.localeCompare(right.tokenHash),
                    ),
                ...
                revokedRefreshTokenFamilies: [...revokedRefreshTokenFamilies.entries()]
                    ...
                    .sort(
                        (left, right) =>
                            left.expiresAt - right.expiresAt || left.familyId.localeCompare(right.familyId),
                    ),
            };
    ...
    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    async function ensureRegisteredClientsLoaded(issuerConfig) {
        const configKey = devOAuthPersistenceConfigKey(issuerConfig);
        ...
        registeredClientsLoadPromise ??= loadRegisteredClients(issuerConfig);
        await registeredClientsLoadPromise;
    }

    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    async function loadRegisteredClients(issuerConfig) {
        const storage = getDevOAuthStorage(issuerConfig);
        registeredClients.clear();
        registeredClientsLoaded = false;
        registeredClientsLoadedFromFile = false;
        ...
        try {
            ...
            const parsed = JSON.parse(text);
            for (const client of parseRegisteredClientRecords(parsed, issuerConfig))
     ...
     */
    function parseRegisteredClientRecords(parsed, issuerConfig) {
        if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return [];
        const root = /** @type {Record<string, unknown>} */ (parsed);
        ...
        for (const item of rawClients) {
            ...
            const expiresAt = Number.isFinite(rawExpiresAt)
                ? rawExpiresAt
                : deriveRegisteredClientExpiresAt(createdAt, issuerConfig);
    ...
    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    async function persistRegisteredClients(issuerConfig) {
        const storage = getDevOAuthStorage(issuerConfig);
        ...
        try {
            const body = {
                ...
                clients: [...registeredClients.values()]
                    ...
                    .sort(
                        (left, right) =>
                            left.createdAt - right.createdAt || left.clientId.localeCompare(right.clientId),
                    ),
            };
     ...
     * @returns {Promise<void>}
     */
    async function handleUserInfo(req, res, config, issuerConfig) {
    ...
    async function readRequestBody(req, options) {
        ...
        if (contentType === FORM_CONTENT_TYPE) {
            if (!options.allowForm) {
                throw oauthRequestBodyError(
                    'application/x-www-form-urlencoded is not supported for this endpoint.',
                    415,
                );
            }
     ...
     * @returns {DevOAuthClient | undefined}
     */
    function resolveTrustedChatGptClientMetadataDocument(clientId, issuerConfig) {
        if (!isTrustedChatGptCimdFallbackEnabled(issuerConfig)) return undefined;
        if (!clientId || clientId.length > MAX_CLIENT_ID_LENGTH || hasControlCharacters(clientId)) return undefined;
        try {
            const url = new URL(clientId);
            if (
    ...
    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    function isTrustedChatGptCimdFallbackEnabled(issuerConfig) {
        return issuerConfig.trustedClients.chatGptCimdFallback;
    }
     ...
     * @returns {DevOAuthClient | undefined}
     */
    function resolveTrustedClaudeClientMetadataDocument(clientId, issuerConfig) {
        if (!isTrustedClaudeCimdFallbackEnabled(issuerConfig)) return undefined;
        if (!clientId || clientId.length > MAX_CLIENT_ID_LENGTH || hasControlCharacters(clientId)) return undefined;
        try {
            const url = new URL(clientId);
    ...
    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    function isTrustedClaudeCimdFallbackEnabled(issuerConfig) {
        return issuerConfig.trustedClients.claudeCimdFallback;
    }

    /** @param {import('./config.js').DevOAuthProcessConfig} issuerConfig */
    function isTrustedChatGptCimdFastPathEnabled(issuerConfig) {
        return issuerConfig.trustedClients.chatGptCimdFastPath;
    }
     ...
     * @returns {Promise<DevOAuthClient | undefined>}
     */
    async function resolveClientMetadataDocument(clientId, issuerConfig) {
        const nowMs = Date.now();
        ...
        const trustedChatGptFallback = resolveTrustedChatGptClientMetadataDocument(clientId, issuerConfig);
        if (trustedChatGptFallback && isTrustedChatGptCimdFastPathEnabled(issuerConfig)) {
        ...
        const trustedClaudeFallback = resolveTrustedClaudeClientMetadataDocument(clientId, issuerConfig);
        if (trustedClaudeFallback && isTrustedClaudeCimdFallbackEnabled(issuerConfig)) {
    ...
    function isAllowedClientMetadataUrl(value) {
        try {
            ...
            if (url.protocol !== 'https:' || url.username || url.password || url.pathname === '/' || url.hash)
                return false;
    ...
    return Object.freeze({
        config: boundProcessConfig,
        isEnabled: isBuiltInDevOAuthEnabled,
        buildMetadata: buildBuiltInDevOAuthMetadata,
        buildClientMetadata: buildBuiltInDevOAuthClientMetadata,
        handleRequest: handleBuiltInDevOAuthRequest,
        readTokenLifetimePolicy: readDevOAuthTokenLifetimePolicy,
        readClientLifetimePolicy: readDevOAuthClientLifetimePolicy,
        readPersistenceConfig: readDevOAuthPersistenceConfig,
