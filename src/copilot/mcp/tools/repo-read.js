...
export const repoReadTools = [
    ...
    },
    {
        ...
        handler: async ({ path, recursive, depth, maxEntries, showHidden }) => {
            const resolved = await resolveReadPath(path ?? 'src/copilot');
    ...
    },
    {
        ...
        description: 'Read a UTF-8 file inside the workspace, optionally using a line window.',
        inputSchema: {
    ...
    },
    {
        ...
        annotations: readOnlyAnnotations(),
        handler: async ({ pattern, path, isRegex, caseSensitive, includePattern, excludePattern, maxResults }) => {
            const resolved = await resolveReadPath(path ?? 'src/copilot');
            if (!resolved.ok) return errorResult(resolved.reason);
