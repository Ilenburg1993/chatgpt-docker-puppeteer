 ...
 * JSON-RPC payloads.
 *
 * @module copilot/mcp/transport/http/stateful/events/store
 */
...
/**
 * @typedef {{ send: (message: import('@modelcontextprotocol/server').JSONRPCMessage) => Promise<void> | void }} McpEventReplaySink
 *
 * @typedef {{
 *     eventId: string;
 *     streamId: string;
 *     sequence: number;
 *     message: import('@modelcontextprotocol/server').JSONRPCMessage;
 *     createdAtMs: number;
 *     expiresAtMs: number;
 * }} McpStoredEvent
 *
...
export function createSqliteMcpEventStore(options) {
    const db = options?.db;
    if (!db) throw new Error('createSqliteMcpEventStore requires an injected database capability.');
...
/**
 * @param {unknown} value
 * @returns {import('@modelcontextprotocol/server').JSONRPCMessage | null}
 */
function parseJsonRpcMessage(value) {
    try {
        const parsed = JSON.parse(String(value ?? 'null'));
        return parsed && typeof parsed === 'object'
            ? /** @type {import('@modelcontextprotocol/server').JSONRPCMessage} */ (parsed)
            : null;
    } catch {
        return null;
    }
