 ...
 *
 * @module copilot/mcp/transport/http/stateful/session/store
 */

/**
 * @typedef {'active' | 'closing' | 'terminated' | 'expired' | 'close_failed'} McpHttpStoredSessionStatus
 *
 ...
 *     beginSessionClose(sessionIdHash: string, reason: McpHttpSessionTerminateReason): void;
 *     completeSessionClose(sessionIdHash: string, closedAtMs: number, reason: McpHttpSessionTerminateReason): void;
 *     failSessionClose(sessionIdHash: string, failedAtMs: number, reason: McpHttpSessionTerminateReason): void;
...
export function createSqliteMcpHttpSessionStoreForDb(db) {
    ...
    return {
        ...
        beginSessionClose(sessionIdHash, reason) {
            db.prepare(
                `UPDATE copilot_mcp_http_sessions
                 SET status = 'closing', terminate_reason = ?, terminated_at_ms = NULL
                 WHERE session_id_hash = ? AND status = 'active'`,
            ).run(reason, sessionIdHash);
        },
        completeSessionClose(sessionIdHash, closedAtMs, reason) {
            db.prepare(
                `UPDATE copilot_mcp_http_sessions
            ...
            ).run(reason, closedAtMs, reason, sessionIdHash);
        },
        failSessionClose(sessionIdHash, failedAtMs, reason) {
            db.prepare(
                `UPDATE copilot_mcp_http_sessions
                 SET status = 'close_failed', terminated_at_ms = ?, terminate_reason = ?
                 WHERE session_id_hash = ?`,
            ).run(failedAtMs, reason, sessionIdHash);
        },
...
export function ensureMcpHttpSessionStoreSchema(db) {
    const existing = db
        .prepare("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'copilot_mcp_http_sessions'")
        .get();
    const existingSql = existing ? String(/** @type {Record<string, unknown>} */ (existing)['sql'] ?? '') : '';
    if (existingSql && !existingSql.includes("'close_failed'")) {
        migrateMcpHttpSessionStoreLifecycleSchema(db);
    }
    db.exec(buildMcpHttpSessionStoreSchemaSql());
}

/**
 * Rebuild the early session table so lifecycle state can distinguish closing and close failure from
 * successful termination. Existing active/terminated/expired records are preserved byte-for-byte at
 * the metadata level.
 *
 * @param {import('#copilot/infra/public/database/sqlite').SqliteDatabasePort} db
 * @returns {void}
 */
function migrateMcpHttpSessionStoreLifecycleSchema(db) {
    db.exec(`
        BEGIN IMMEDIATE;
        DROP INDEX IF EXISTS idx_mcp_http_sessions_status;
        DROP INDEX IF EXISTS idx_mcp_http_sessions_last_seen;
        ALTER TABLE copilot_mcp_http_sessions RENAME TO copilot_mcp_http_sessions_pre_lifecycle;
        ${buildMcpHttpSessionStoreTableSql()}
        INSERT INTO copilot_mcp_http_sessions (
            session_id_hash, session_id_preview, protocol_version, created_at_ms, last_seen_at_ms,
            expires_at_ms, status, terminated_at_ms, terminate_reason, auth_binding_json, transport_json
        )
        SELECT
            session_id_hash, session_id_preview, protocol_version, created_at_ms, last_seen_at_ms,
            expires_at_ms, status, terminated_at_ms, terminate_reason, auth_binding_json, transport_json
        FROM copilot_mcp_http_sessions_pre_lifecycle;
        DROP TABLE copilot_mcp_http_sessions_pre_lifecycle;
        ${buildMcpHttpSessionStoreIndexesSql()}
        COMMIT;
    `);
}

/** @returns {string} */
function buildMcpHttpSessionStoreSchemaSql() {
    return `${buildMcpHttpSessionStoreTableSql()}\n${buildMcpHttpSessionStoreIndexesSql()}`;
}

/** @returns {string} */
function buildMcpHttpSessionStoreTableSql() {
    return `CREATE TABLE IF NOT EXISTS copilot_mcp_http_sessions (
        ...
        status             TEXT NOT NULL CHECK(status IN ('active', 'closing', 'terminated', 'expired', 'close_failed')),
...
/** @returns {string} */
function buildMcpHttpSessionStoreIndexesSql() {
    return `CREATE INDEX IF NOT EXISTS idx_mcp_http_sessions_status
