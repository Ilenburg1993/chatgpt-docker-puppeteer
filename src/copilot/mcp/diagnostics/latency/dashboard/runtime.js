// @ts-check
/** Latency dashboard assessment, ranking, accounting and history runtime. */

import { readMcpMetricsSnapshot } from '#copilot/mcp/public/observability';
import { readMcpRoundTripAnalyticsSnapshot } from '../round-trip/public/index.js';
import {
 ...
 */
/** @typedef {{
 *     policy: McpLatencyDashboardPolicy;
 *     sessionRuntimeState?: Record<string, unknown>;
 *     readRoundTripSnapshot?: typeof readMcpRoundTripAnalyticsSnapshot;
 *     metrics?: ReturnType<typeof readMcpMetricsSnapshot>;
 * }} McpLatencyDashboardRuntime */
...
function compactRoundTripAnalytics(snapshot) {
    return {
        ...
        optimizationEvidence: {
            newCompositeRecommendation: snapshot.optimizationEvidence.newCompositeRecommendation,
            existingMechanisms: snapshot.optimizationEvidence.existingMechanisms.slice(0, 3),
        },
        workflowPressure: {
...
async function buildLatencyHistoryReport(dashboard, options) {
    ...
    const comparison = compareMcpLatencyDashboardSnapshots(
        /** @type {import('./history.js').McpLatencyDashboardSnapshot} */ (dashboard),
        previousSnapshot,
    );
 ...
 * @param {import('../config.js').McpLatencyDashboardPolicy} policy
 * @returns {LatencyDashboardBudgets}
 */
function readLatencyDashboardBudgets(options, policy) {
    return {
        minSampleCalls: readBoundedInteger(options['minSampleCalls'], DEFAULT_MIN_SAMPLE_CALLS, 1, 10_000),
        toolAverageWarnMs: policy.toolAverageWarnMs,
        authorizationAverageWarnMs: policy.authorizationAverageWarnMs,
        handlerAverageWarnMs: policy.handlerAverageWarnMs,
        resultSizeAverageWarnMs: policy.resultSizeAverageWarnMs,
        errorRateWarn: policy.errorRateWarn,
        silentExternalGapP50WarnMs: readBoundedInteger(
            options['silentExternalGapP50WarnMs'],
            policy.silentExternalGapP50WarnMs,
            100,
            120_000,
        ),
        silentExternalGapP95WarnMs: readBoundedInteger(
            options['silentExternalGapP95WarnMs'],
            policy.silentExternalGapP95WarnMs,
            100,
            120_000,
        ),
    };
...
/**
 * Build one latency dashboard snapshot from explicit policy/capability projections.
 * @param {McpLatencyDashboardRuntime} runtime
 * @param {Record<string, unknown>} [input]
 * @returns {Promise<Record<string, unknown>>}
 */
export async function buildMcpLatencyDashboard(runtime, input = {}) {
    const options = /** @type {Record<string, unknown>} */ (input);
    const policy = runtime.policy;
    const budgets = readLatencyDashboardBudgets(options, policy);
    const maxRows = readBoundedInteger(options['maxRows'], MAX_ROWS, 1, 50);
    const includeTools = options['includeTools'] === true;
    const rankingRows = includeTools ? maxRows : 1;
    const metrics = runtime.metrics ?? readMcpMetricsSnapshot();
    const sessionRuntime = runtime.sessionRuntimeState ?? {
        available: false,
        reason: 'http-session-runtime-not-owned-by-current-transport',
    };
    const readRoundTripSnapshot = runtime.readRoundTripSnapshot ?? readMcpRoundTripAnalyticsSnapshot;
    const indexedRoundTrip = readRoundTripSnapshot({
