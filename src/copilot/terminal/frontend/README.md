...
> Como o terminal lê e executa operações do runtime **sem** conhecer a topologia interna do agent, SDK, hub ou transportes?
...
- `commands/`, `repl.js` e `dialog/` devem preferir esta pasta quando precisarem **consumir** runtime.
- Esta pasta pode ampliar a ergonomia do runtime, mas sempre usando `agent/` + `sdk/` como base canônica.
- Não deve recriar semântica do SDK (ex.: mode/plan, streaming, usage).
- `sdk-session-projection.js` é a fonte preferida quando o problema for especificamente `mode/plan` vanilla.

## Heurística prática

- Se o problema é **“preciso de uma projeção pronta para a UX”**, provavelmente entra aqui.
- Se o problema é **“preciso renderizar/mostrar/enviar algo no REPL”**, provavelmente entra em `terminal/dialog/`.
