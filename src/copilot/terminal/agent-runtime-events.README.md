...
- eventos vanilla da sessão SDK como `session.mode_changed`, `session.plan_changed`,
  `session.truncation`, `session.workspace_file_changed`, `assistant.turn_start`, etc.

Esses pertencem a `sdk-session-events.js`.

## Regra prática

Se o payload nasce no SDK, prefira `sdk-session-events.js`. Se o payload já foi estabilizado pelo
runtime/agent, use este módulo.
