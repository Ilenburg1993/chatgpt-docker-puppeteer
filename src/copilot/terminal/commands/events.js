...
/**
 * @param {string} arg
 * @returns {{ limit: number; event: string | null; traceId: string | null; turnId: string | null; source: string | null }}
 */
function parseEventsArg(arg) {
    ...
    return { limit, event, traceId, turnId, source };
}
...
function summarizePayload(payload) {
    ...
    return [toolName ? `tool=${compact(toolName)}` : null, status ? `status=${compact(status)}` : null, content ? compact(content) : null]
        .filter(Boolean)
        .join(' · ');
}
...
export async function cmdEvents({ println }, arg = '') {
    ...
    for (const entry of entries) {
        ...
        println(
            `    \x1b[90m${time}\x1b[0m  #${entry.eventId} \x1b[33m${entry.event}\x1b[0m  \x1b[90m${origin}${trace}${turn}\x1b[0m${summary ? ` — ${summary}` : ''}`,
        );
    }
    println('');
}
