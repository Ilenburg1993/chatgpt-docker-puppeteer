 ...
 *
 * Exporta o typedef `IAlwaysAliveAgent` — a API pública mínima que qualquer consumer do agente deve
 * utilizar, evitando acoplamento direto à implementação concreta.
 *
 * Uso recomendado:
 * ```js
 ...
 *
 * Substitui `AlwaysAliveAgentLike` dispersa em `bridge-control.js`. Centralizar o typedef aqui faz
 * com que alterações na API pública do agente reflitam em todos os consumers.
 *
 ...
 * @property {(message: string, opts?: { timeoutMs?: number; attachments?: any }) => Promise<unknown>} sendMessage - Envia mensagem ao agente
 ...
 * @property {(text: string, opts?: { timeout?: number }) => Promise<string>} sendDialogTurn - Envia um turn ao dialog loop
 * @property {(opts?: { authorized?: boolean; reason?: 'watchdog_restart' | 'authorized_stop' }) => Promise<void>} stopDialogLoop - Para o dialog loop
 * @property {(() => 'approve_all' | 'audit_only' | 'selective') | undefined} getPermissionMode - Retorna o modo de permissão atual
 * @property {((mode: 'approve_all' | 'audit_only' | 'selective', opts?: { allowTools?: string[]; denyTools?: string[]; denyShell?: boolean }) => void) | undefined} setPermissionMode - Define o modo de permissão
