 ...
 *
 * Define a interface mínima que um Agent plugin deve implementar para ser registrado e
 * utilizado pelo sistema. O {@link AlwaysAliveAgent} satisfaz este contrato.
 *
 ...
 *
 * Agents são componentes de longa duração que mantêm sessão com o Copilot SDK,
 * processam mensagens, gerenciam tools e emitem eventos.
 *
 * @typedef {Object} AgentPlugin
 * @property {'stopped' | 'starting' | 'idle' | 'processing' | 'cooldown'} status
 *   Estado do ciclo de vida do agente.
 * @property {() => Promise<void>} start
 *   Inicializa o agente: cria/restaura sessão, registra tools, inicia event loop.
 * @property {(opts?: { shutdownTimeoutMs?: number }) => Promise<void>} stop
 *   Encerra o agente gracefully: flush de pendências, persiste estado, libera recursos.
 * @property {(message: string, opts?: SendMessageOptions) => Promise<string>} sendMessage
 *   Envia uma mensagem ao LLM e retorna a resposta. Enfileira se já houver mensagem em processamento.
 * @property {() => AgentStatusSnapshot} getStatusSnapshot
 *   Retorna snapshot do estado atual do agente (modelo, sessão, métricas, etc.).
 * @property {(event: string, listener: (...args: any[]) => void) => void} on
 *   Registra listener para evento do agente.
 * @property {(event: string, listener: (...args: any[]) => void) => void} off
 *   Remove listener de evento do agente.
 * @property {(event: string, ...args: any[]) => boolean} emit
 *   Emite evento do agente.
 */
 ...
 *
 * @typedef {'ready' | 'task.queued' | 'task.delta' | 'task.done' | 'task.error' | 'agent.metrics' | 'agent.stopped' | 'session.error'} AgentEvent
 */
