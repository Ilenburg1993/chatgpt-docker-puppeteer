 ...
 *
 * O domínio de probes não deve conhecer `CopilotSession`: ele só precisa abrir uma sessão descartável, observar eventos,
 * enviar uma mensagem e, no caminho de erro do chat probe, abortar a mensagem ativa. O adapter default concentra os
 * casts para a implementação concreta do SDK numa única borda.
 *
 ...
 *   subscribe: (
 *     session: unknown,
 *     handlers: Parameters<typeof onSessionEvents>[1],
 *   ) => () => void;
 *   sendAndWait: (
 *     session: unknown,
...
export const DEFAULT_MODEL_GATEWAY_PROBE_SESSION_RUNTIME = Object.freeze({
    ...
    subscribe(session, handlers) {
        return onSessionEvents(
            /** @type {Parameters<typeof onSessionEvents>[0]} */ (session),
            handlers,
        );
    },
