 ...
 *
 * AH.4 — Fábrica de Custom Tools para o Always-Alive Agent.
 * Encapsula `defineTool` do SDK com convenções do projeto:
 ...
 *
 * Todas as tools criadas com `buildTool` são expostas ao motor LSP (Pylance/tsserver) via
 * geração de JSON Schema inline no `parameters`. O fluxo é:
 *
 * 1. **Zod schema** (recomendado): passa um `ZodTypeAny` como `parameters`.
 *    O `zodToJsonSchema` converte para um JSON Schema Draft 7, que o SDK usa para:
 *    - Gerar a especificação `tool_description` enviada ao modelo
 *    - Validar argumentos recebidos antes de invocar o `handler`
 *
 * 2. **JSON Schema manual**: passa um `Record<string, unknown>` diretamente.
 *    Útil quando a ferramenta aceita tipos dinâmicos ou não-Zod.
 *
 ...
 * @property {import('zod/v3').ZodTypeAny | Record<string, unknown>} [parameters] - Schema Zod ou JSON Schema manual dos parâmetros
...
/**
 * Normaliza o schema de parâmetros para o formato aceito pelo SDK.
 * Aceita instâncias Zod (convertidas automaticamente) ou JSON Schema direto.
 *
...
function normalizeParameters(parameters) {
    ...
    if ('_def' in parameters) {
        try {
            return /** @type {Record<string, unknown>} */ (zodToJsonSchema(/** @type {import('zod/v3').ZodTypeAny} */ (parameters)));
        } catch (err) {
 ...
 * });
 */
export function buildTool({ name, description, parameters, handler, requiresApproval = true, overridesBuiltInTool = false }) {
    const jsonSchemaParams = normalizeParameters(parameters);

    const wrappedHandler = /** @type {import('@github/copilot-sdk').ToolHandler<TArgs>} */ (async (args, invocation) => {
