...
import { readWorkspaceInfraConfig } from '../workspace/config/index.js';
...
export function readInfraConfig(env, overrides = {}) {
    ...
    return Object.freeze({
        ...
        index: readIoIndexRuntimeConfig(/** @type {NodeJS.ProcessEnv} */ (env)),
...
/**
 * Apply runtime-local composition overrides without consulting the environment again.
 * @param {ReturnType<typeof readInfraConfig>} config
 * @param {{mutationAuditLogPath?:string|null}} [overrides]
 */
export function withInfraRuntimeConfigOverrides(config, overrides = {}) {
    if (overrides.mutationAuditLogPath === undefined) return config;
    return Object.freeze({
        ...config,
        mutationAudit: Object.freeze({ filePath: normalizeConfiguredPath(overrides.mutationAuditLogPath) }),
    });
}
