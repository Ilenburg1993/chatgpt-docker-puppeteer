...
import { iterateLineChunks } from './content.js';
...
/**
 * @param {{ exec: Function; transaction?: Function; inTransaction?: boolean }} db
 * @param {() => void} operation
 */
function runSchemaTransaction(db, operation) {
    if (db.inTransaction) {
        operation();
        return;
    }
...
function backfillLegacyChunks(db) {
    ...
    for (const row of rows) {
        for (const chunk of iterateLineChunks(String(row.content ?? ''))) {
            insertChunk.run(
                row.filePath,
                chunk.index,
                chunk.startLine,
 ...
 *
 * @param {{ exec: Function; prepare: Function; transaction?: Function; inTransaction?: boolean }} db
 * @returns {number}
 */
