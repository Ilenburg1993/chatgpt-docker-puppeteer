...
import { warmFromDirectory } from '../prefetch/index.js';
 ...
 */
export async function selectAndWarmScopePaths(scope, opts, signal, runtime) {
    ...
    if (directory) {
        ...
        for (const seedSymbol of uniqueSeedSymbols) {
            const rows =
                runtime.indexRegistry?.findSymbol(seedSymbol, {
        ...
        const warm = await runtime.prefetchSessions.start(opts.sessionId, explicitPaths, {
            concurrency,
            silent,
            captureTextSnapshots: true,
            cacheBytes: false,
