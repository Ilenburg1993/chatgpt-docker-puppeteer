 ...
 *
 * @module copilot/infra/platform/process-output/capture
 */

import { concatBufferViews, decodeUtf8Buffer, toBufferView } from '../buffer/index.js';
...
export class BoundedProcessOutputCapture {
    ...
    constructor(options = {}) {
        ...
        this.#buffer = Buffer.allocUnsafe(this.#mode === 'tail' ? maxBytes : Math.min(maxBytes, 64 * 1024));
    }
    ...
    append(chunk) {
        ...
        const acceptedBytes = this.#mode === 'tail' ? this.#appendTail(bytes) : this.#appendHead(bytes);
        return {
            acceptedBytes,
    ...
    toBuffer() {
        ...
        return concatBufferViews(
            [this.#buffer.subarray(this.#writeOffset), this.#buffer.subarray(0, this.#writeOffset)],
            this.#storedBytes,
        );
    }
     ...
     */
        ...
        const nextCapacity = Math.min(this.#maxBytes, Math.max(requiredBytes, this.#buffer.byteLength * 2));
    ...
    #appendTail(bytes) {
        if (bytes.byteLength >= this.#buffer.byteLength) {
            bytes.copy(this.#buffer, 0, bytes.byteLength - this.#buffer.byteLength, bytes.byteLength);
