...
import {
    BYTE_LINE_INDEX_MAX_ENTRIES,
    estimateByteLineIndexEntryBytes,
    readByteLineIndexMaxBytes,
    readByteLineIndexMaxLines,
} from './policy.js';
 ...
 * }} state
 */
export function rememberByteLineIndexStreamSeed(filePath, state) {
    if (
 ...
 */
export async function getByteLineIndex(filePath, options = {}) {
...
export function recordByteLineIndexCapturedRangeReuse(bytesAvoided) {
...
export function recordByteLineIndexRangeRead(bytesRead) {
    byteLineIndexStats.rangeBytesRead += Math.max(0, bytesRead);
}

/** @param {string} filePath */
export function discardStaleByteLineIndex(filePath) {
