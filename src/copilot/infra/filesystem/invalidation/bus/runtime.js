 ...
 *
 * @module copilot/infra/filesystem/invalidation/bus/runtime
 */

import { invalidateIoPathPolicyCache, registerShutdownHandler, SHUTDOWN_PRIORITY } from '#copilot/core';
import { getIoL2Cache, invalidateIoCachePath, invalidateIoCacheSubtree } from '#copilot/infra/internal/cache';
import {
...
/** @type {(() => void) | null} */
let _stopCrossProcessConsumer = null;
...
function dispatchLocalInvalidation(filePath, normalized) {
    ...
    try {
        if (normalized.recursive) invalidateIoCacheSubtree(filePath);
        else invalidateIoCachePath(filePath);
    } catch {
        /* L1 invalidation é best-effort e nunca interrompe a mutação canônica. */
    }
    try {
        getIoL2Cache()?.invalidatePath(filePath);
    } catch {
        /* L2 invalidation é best-effort e nunca interrompe a mutação canônica. */
    }
    try {
...
function ensureCrossProcessConsumer() {
    // Idempotent and deliberately retryable. A module may be used before the boot composition root configures SQLite;
    // that early attempt must never poison later initialization once the provider becomes available.
    _stopCrossProcessConsumer = startCrossProcessInvalidationConsumer((filePath, event) => {
...
function ensureShutdownFlushHook() {
    ...
    registerShutdownHandler(
        'copilot-infra.invalidation.flush-stop',
        async () => {
            flushIoInvalidationQueue();
            _stopCrossProcessConsumer?.();
            _stopCrossProcessConsumer = null;
        },
