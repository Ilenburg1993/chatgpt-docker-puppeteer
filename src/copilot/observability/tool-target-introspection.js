 ...
 *
 * @module copilot/observability/tool-target-introspection
 */
 ...
 * @returns {void}
 */
function visitStructuredMetadata(value, state, depth, seen = new WeakSet()) {
    if (depth > MAX_RECURSION_DEPTH) return;
    if (Array.isArray(value)) {
        if (seen.has(value)) return;
        seen.add(value);
        for (const item of value) visitStructuredMetadata(item, state, depth + 1, seen);
        return;
    }
    ...
    for (const [key, raw] of Object.entries(value)) {
        ...
        visitStructuredMetadata(raw, state, depth + 1, seen);
    }
}
 ...
 * @returns {void}
 */
function walkTextNodes(value, onText, depth, seen = new WeakSet()) {
    if (depth > MAX_RECURSION_DEPTH) return;
    ...
    if (Array.isArray(value)) {
        if (seen.has(value)) return;
        seen.add(value);
        for (const item of value) walkTextNodes(item, onText, depth + 1, seen);
        return;
    }
    if (!isObjectRecord(value)) return;
    if (seen.has(value)) return;
    seen.add(value);
    for (const nested of Object.values(value)) walkTextNodes(nested, onText, depth + 1, seen);
}
...
/**
 * @param {{ toolName?: string; args?: unknown; result?: unknown }} input
 * @returns {ToolTargetIntrospection}
 */
export function introspectToolTargets({ toolName, args, result }) {
    /** @type {Set<string>} */
    ...
    if (
        ...
        typeof args === 'object' &&
        !Array.isArray(args) &&
        /(?:copy|move|rename|transfer).*file|file.*(?:copy|move|rename|transfer)/iu.test(String(toolName ?? ''))
    ) {
        const source = asCleanString(/** @type {Record<string,unknown>} */ (args)['source']);
        if (source && !isLikelyUrl(source)) addIfPresent(fileTargets, source);
    }
