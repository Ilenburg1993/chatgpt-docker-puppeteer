 ...
 *
 * EventBus subscriber que rastreia correlationIds em eventos, construindo
 * um trace log para debug end-to-end. Complementa o correlationEnricher
 * middleware (FAIXA-L16) que injeta o ID — este módulo observa e indexa.
 *
 ...
 * @returns {{ unsub: () => void; hasAction: true; name: string; getTraces: (correlationId: string) => TraceEntry[]; getRecentTraces: (limit?: number) => TraceEntry[] }}
 */
export function createCorrelationTracer({ bus, maxTraces = 500 }) {
    ...
    unsubs.push(bus.on('*', (/** @type {any} */ evt) => {
        const entry = {
