...
export function createMetricsCollector({ bus, metrics }) {
    ...
    on('model_gateway:route:decision', (evt) => {
        ...
        metrics.recordCounter(
            evt.selected === false ? 'model_gateway.route.unselected' : 'model_gateway.route.selected',
            1,
        );
        ...
        metrics.recordGauge(
            'model_gateway.route.fallback',
            Array.isArray(evt.fallbackChain) ? evt.fallbackChain.length : 0,
        );
    });
