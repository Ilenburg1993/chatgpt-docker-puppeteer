 ...
 *
 * Schemas built-in para TODOS os 122 tipos de evento bus SSOT do sistema.
 * Cobertura: 100% dos bus events.
 *
...
export const BUILTIN_SCHEMAS = [
    ...
    { type: 'agent:error', required: ['type', 'timestamp'], fields: { message: 'string' }, description: 'Erro genérico do agent' },
    ...
    // ── Assistant ────────────────────────────────────────
    { type: 'agent:assistant:intent', required: ['type', 'timestamp'], description: 'Intenção detectada do assistente' },
    { type: 'agent:assistant:reasoning_complete', required: ['type', 'timestamp'], description: 'Raciocínio do assistente concluído' },
    { type: 'agent:assistant:turn_end', required: ['type', 'timestamp'], description: 'Fim de turno do assistente' },
    { type: 'agent:assistant:turn_start', required: ['type', 'timestamp'], description: 'Início de turno do assistente' },
    ...
    // ── Context ──────────────────────────────────────────
    { type: 'agent:context:compacted', required: ['type', 'timestamp'], description: 'Contexto compactado com sucesso' },

    // ── Dialog ───────────────────────────────────────────
    { type: 'agent:dialog:boot_recovery', required: ['type', 'timestamp'], description: 'Recuperação de boot do dialog' },
    { type: 'agent:dialog:compaction:requested', required: ['type', 'timestamp'], description: 'Compactação do dialog solicitada' },
    { type: 'agent:dialog:delta', required: ['type', 'timestamp'], description: 'Delta incremental do dialog' },
    { type: 'agent:dialog:loop:changed', required: ['type', 'timestamp'], fields: { active: 'boolean' }, description: 'Estado do dialog loop mudou (ativo/inativo)' },
    { type: 'agent:dialog:paused', required: ['type', 'timestamp'], description: 'Dialog pausado' },
    { type: 'agent:dialog:pre_stall_warning', required: ['type', 'timestamp'], fields: { stalledMs: 'number' }, description: 'Alerta prévio de stall no dialog' },
    ...
    // ── Elicitation ──────────────────────────────────────
    { type: 'agent:elicitation:pending', required: ['type', 'timestamp'], description: 'Elicitação pendente de resposta do usuário' },

    // ── Emitter ──────────────────────────────────────────
    { type: 'agent:emitter:error', required: ['type', 'timestamp'], fields: { message: 'string' }, description: 'Erro interno no emitter' },

    // ── Exit Plan Mode ───────────────────────────────────
    { type: 'agent:exit_plan_mode:completed', required: ['type', 'timestamp'], description: 'Saída do plan mode concluída' },

    // ── External Tool ────────────────────────────────────
    { type: 'agent:external_tool:completed', required: ['type', 'timestamp'], description: 'Ferramenta externa concluída' },

    // ── Handoff ──────────────────────────────────────────
    { type: 'agent:handoff:accepted', required: ['type', 'timestamp'], description: 'Handoff aceito' },
    { type: 'agent:handoff:received', required: ['type', 'timestamp'], description: 'Handoff recebido de outro agente' },
    { type: 'agent:handoff:rejected', required: ['type', 'timestamp'], fields: { rejectReason: 'string' }, description: 'Handoff rejeitado' },
    ...
    // ── Pending Messages ─────────────────────────────────
    { type: 'agent:pending_messages:modified', required: ['type', 'timestamp'], description: 'Mensagens pendentes modificadas' },

    // ── Permission ───────────────────────────────────────
    { type: 'agent:permission:mode_changed', required: ['type', 'timestamp'], fields: { mode: 'string' }, description: 'Modo de permissão alterado' },

    // ── PR ────────────────────────────────────────────────
    { type: 'agent:pr:consumed', required: ['type', 'timestamp'], description: 'PR consumido pelo agent' },
    { type: 'agent:pr:fallback_model', required: ['type', 'timestamp'], description: 'Fallback de modelo na geração de PR' },

    // ── Question ─────────────────────────────────────────
    { type: 'agent:question:answered', required: ['type', 'timestamp'], description: 'Pergunta respondida pelo usuário' },
    { type: 'agent:question:pending', required: ['type', 'timestamp'], description: 'Pergunta pendente para o usuário' },
    ...
    { type: 'agent:session:compaction_complete', required: ['type', 'timestamp'], description: 'Compactação de sessão concluída' },
    { type: 'agent:session:compaction_start', required: ['type', 'timestamp'], description: 'Início de compactação de sessão' },
    { type: 'agent:session:context_changed', required: ['type', 'timestamp'], description: 'Contexto da sessão alterado' },
    { type: 'agent:session:error', required: ['type', 'timestamp'], fields: { message: 'string' }, description: 'Erro de sessão' },
    { type: 'agent:session:fatal', required: ['type', 'timestamp'], fields: { message: 'string' }, description: 'Erro fatal de sessão' },
    { type: 'agent:session:handoff', required: ['type', 'timestamp'], description: 'Handoff de sessão' },
    { type: 'agent:session:history_synced', required: ['type', 'timestamp'], description: 'Histórico de sessão sincronizado' },
    { type: 'agent:session:idle', required: ['type', 'timestamp'], description: 'Sessão ociosa, pronta para próximo comando' },
    ...
    { type: 'agent:session:snapshot_rewind', required: ['type', 'timestamp'], description: 'Rewind de snapshot da sessão' },
    ...
    { type: 'agent:session:token_budget_warning', required: ['type', 'timestamp'], description: 'Aviso de budget de tokens' },
    ...
    { type: 'agent:session:workspace_file_changed', required: ['type', 'timestamp'], description: 'Arquivo do workspace alterado' },

    // ── Shell ────────────────────────────────────────────
    { type: 'agent:shell:completed', required: ['type', 'timestamp'], description: 'Comando shell concluído' },
    { type: 'agent:shell:detached_completed', required: ['type', 'timestamp'], description: 'Comando shell detached concluído' },
    ...
    { type: 'agent:subagent:failed', required: ['type', 'timestamp'], fields: { message: 'string' }, description: 'Subagent falhou' },
    ...
    { type: 'agent:task:error', required: ['type', 'timestamp'], fields: { message: 'string' }, description: 'Erro em task' },
    ...
    // ── Tool ─────────────────────────────────────────────
    { type: 'agent:tool:execution_complete', required: ['type', 'timestamp'], fields: { tool: 'string' }, description: 'Execução de ferramenta concluída' },
    { type: 'agent:tool:execution_progress', required: ['type', 'timestamp'], fields: { tool: 'string' }, description: 'Progresso de execução de ferramenta' },
    { type: 'agent:tool:execution_start', required: ['type', 'timestamp'], fields: { tool: 'string' }, description: 'Início de execução de ferramenta' },

    // ═══════════════════════════════════════════════════════════════════
    // HOOK EVENTS (6)
    // ═══════════════════════════════════════════════════════════════════
    { type: 'hook:error_occurred', required: ['type', 'timestamp'], fields: { message: 'string' }, description: 'Erro em hook' },
    ...
    // HUB EVENTS (8)
    // ═══════════════════════════════════════════════════════════════════
    { type: 'session:created', required: ['type', 'timestamp'], fields: { sessionId: 'string' }, description: 'Sessão do ConversationHub criada' },
    { type: 'session:closed', required: ['type', 'timestamp'], fields: { sessionId: 'string' }, description: 'Sessão do ConversationHub fechada' },
    ...
    // SERVICE EVENTS (5)
    // ═══════════════════════════════════════════════════════════════════
    { type: 'service:session:created', required: ['type', 'timestamp'], fields: { sessionId: 'string' }, description: 'Sessão de serviço criada' },
    { type: 'service:session:disconnected', required: ['type', 'timestamp'], description: 'Sessão de serviço desconectada' },
    ...
    { type: 'nerv:command:send_message', required: ['type', 'timestamp'], description: 'Comando NERV: enviar mensagem' },
];
