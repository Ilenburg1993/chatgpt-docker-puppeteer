FILE_ORIGINAL: D:\chatgpt-docker-puppeteer\src\driver\modules\analyzer.js
PASTA_BASE: driver
  ...
  const SADI = {
    ...
    query: (selector, root = document, onlyFrames = false, accumulator = []) => {
      ...
      for (const f of frames) {
        try { 
          if (f.contentDocument) SADI.query(selector, f.contentDocument, onlyFrames, accumulator); 
        } catch (e) {}
      }
      return accumulator;
    },
    ...
    getFrameIdentity: (el) => {
        ...
        if (base.length > el.tagName.length) return base;
        
        if (!el.parentNode) return base;
    ...
    isOccluded: (el) => {
      ...
      if (style.display === 'none' || style.visibility === 'hidden' || parseFloat(style.opacity || '1') < 0.1) return true;
      ...
      if (topEl && !el.contains(topEl) && !topEl.contains(el)) return true;
      ...
      if (window !== window.top) {
          try {
              ...
              if (frameEl) {
                  ...
                  if (pTopEl && !frameEl.contains(pTopEl)) return true;
              }
          } catch (e) { }
      }
      return false;
...
async function findFrameByPath(page, framePath) {
    ...
    return frames.find(f => {
        try {
            const fUrl = f.url();
            if (!fUrl || fUrl === 'about:blank') return false;
            const url = new URL(fUrl);
            const validPath = url.pathname.length > 1 ? url.pathname : null;
            if (validPath) return framePath.includes(validPath);
    ...
    return await page.evaluate(async (proto, svgSigs, logicFnStr) => {
        ...
        const scoreButton = (btn) => {
            ...
            for (const p of paths) {
                ...
                if (svgSigs.some(sig => d.startsWith(sig))) { score += 200; break; }
            }
            
            // Atributos de intenção
            if (btn.getAttribute('data-testid')?.includes('send')) score += 150;
...
async function findResponseArea(page) {
    return await page.evaluate(async (logicFnStr) => {
        ...
        const final = best || containers.filter(c => c.isConnected).sort((a,b) => b.innerText.length - a.innerText.length)[0];
        return final ? { 
