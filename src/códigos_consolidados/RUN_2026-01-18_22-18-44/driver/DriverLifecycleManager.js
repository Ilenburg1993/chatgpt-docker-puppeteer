FILE_ORIGINAL: D:\chatgpt-docker-puppeteer\src\driver\DriverLifecycleManager.js
PASTA_BASE: driver
...
const telemetryBridge = require('./modules/telemetry_bridge');
...
class DriverLifecycleManager {
    ...
    async acquire() {
        try {
            ...
            // 3. [V700] ACOPLAMENTO SENSORIAL: Liga os sensores da ponte ao driver ativo
            // Permite que o Dashboard "veja" e "sinta" as micro-ações do robô.
            try {
                telemetryBridge.attach(this.driver);
            } catch (bridgeErr) {
                log('WARN', `[LIFECYCLE] Falha não fatal no acoplamento sensorial: ${bridgeErr.message}`, this.correlationId);
            }
