FILE_ORIGINAL: D:\chatgpt-docker-puppeteer\src\driver\modules\triage.js
PASTA_BASE: driver
  ...
  try {
    const diagnosis = await page.evaluate(async (errors, closers, delayMs, maxParts) => {
      const Probe = {
         ...
         */
        scan: (root = document, acc = { textParts: [], nodeCount: 0, hasPassword: false, spinners: [], buttons: [] }, depth = 0) => {
          ...
          while (node) {
            if (node.nodeType === 1) {
              ...
              if (tag === 'INPUT' && node.type === 'password') acc.hasPassword = true;
              if (node.matches('.spinner, .loading, [aria-busy="true"]')) acc.spinners.push(node);
              if (tag === 'BUTTON' || node.getAttribute('role') === 'button') acc.buttons.push(node);
              ...
              if (acc.textParts.length < maxParts) {
                ...
                if (text.length > 2 || attrs.length > 2) acc.textParts.push(`${text}:${attrs}`);
              }

              if (node.shadowRoot) Probe.scan(node.shadowRoot, acc, depth + 1);
              if (tag === 'IFRAME') {
                try { if (node.contentDocument) Probe.scan(node.contentDocument, acc, depth + 1); } catch {}
              }
            }
        ...
        checkVisualError: (buttons) => {
          const candidates = buttons.filter(b => b.matches('[role="alert"], .error, .warning, [class*="error"]'));
          for (const el of candidates) {
            if (!el.isConnected || el.innerText.length < 3) continue;
            ...
            if (isAlert(fg) || isAlert(bg)) return el.innerText;
          }
          return null;
        }
      };
      ...
      if (fullText.includes('cloudflare') || fullText.includes('captcha') || document.querySelector('[id*="challenge"]')) {
      ...
      const hasMajorBarrier = Array.from(document.querySelectorAll('iframe')).some(f => {
        try { if (f.contentDocument) return false; } catch {
      ...
      if (hasMajorBarrier && snap2.nodeCount < 50) {
        return { type: 'INFRA_BARRIER_DETECTED', severity: 'HIGH', evidence: { type: 'cross_origin_iframe' } };
      }
      ...
      if (visualErrorText) {
        return { type: 'VISUAL_ERROR_DETECTED', severity: 'MEDIUM', evidence: { text: visualErrorText.slice(0, 100) } };
      }

      // 6. FIM ABRUPTO (Botão de Retry sem botão de Stop)
      const retryBtn = snap2.buttons.find(b => closers.some(c => (b.innerText || '').toLowerCase().includes(c)) && b.offsetParent !== null);
      const stopBtn = snap2.buttons.find(b => (b.querySelector('svg rect') || (b.innerText || '').toLowerCase().includes('stop')) && b.offsetParent !== null);
      if (retryBtn && !stopBtn) {
    ...
    return diagnosis || { type: 'HEALTHY', severity: 'NONE', ts: Date.now() };
