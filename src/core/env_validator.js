...
const ENV_SCHEMA = {
    ...
    NODE_ENV: {
        level: 'FATAL',
        validator: (/** @type {any} */ val) => ['development', 'production', 'test'].includes(val),
        default: 'development',
        message: 'Must be one of: development, production, test',
    },
    SERVER_MODE: {
        level: 'FATAL',
        validator: (/** @type {any} */ val) => ['split', 'integrated', 'disabled'].includes(val),
        default: 'split',
        message: 'Must be one of: split, integrated, disabled',
    },
    SERVER_AUTHORITY: {
        level: 'FATAL',
        validator: (/** @type {any} */ val) => ['standalone', 'delegated'].includes(val),
        default: 'standalone',
        message: 'Must be one of: standalone, delegated',
    },
    BROWSER_MODE: {
        level: 'FATAL',
        validator: (/** @type {any} */ val) => ['wsEndpoint', 'connect', 'launcher', 'auto'].includes(val),
        default: 'wsEndpoint',
    ...
    CHROME_HOST: {
        level: 'ERROR',
        validator: (/** @type {any} */ val) => val && val.length > 0,
    ...
    OLLAMA_GENERATE_TIMEOUT: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => !isNaN(parseInt(val, 10)) && parseInt(val, 10) > 0,
        default: '60000',
    ...
    OLLAMA_NON_EMBEDDING_RUNTIME: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => ['auto', 'cloud', 'local'].includes(String(val)),
        default: 'auto',
    ...
    OLLAMA_LOCAL_MODEL_PROFILE: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => ['light', 'custom'].includes(String(val)),
        default: 'light',
        message: 'Must be one of: light, custom',
    },
    OLLAMA_LOCAL_ALLOWED_MODELS: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => typeof val === 'string',
        default: '',
        message: 'Must be a comma-separated string (or empty)',
    },
    RAG_PROFILE_DEFAULT: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => ['core', 'dev', 'full'].includes(String(val)),
        default: 'core',
        message: 'Must be one of: core, dev, full',
    },
    RAG_DOCS_MODE: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => ['include', 'exclude', 'only'].includes(String(val)),
        default: 'include',
    ...
    RAG_INDEX_MAX_FILE_BYTES: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => !isNaN(parseInt(val, 10)) && parseInt(val, 10) >= 1000,
        default: '2000000',
    ...
    RAG_CHUNK_TARGET_CHARS: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => !isNaN(parseInt(val, 10)) && parseInt(val, 10) >= 800,
        default: '1800',
        message: 'Must be an integer >= 800',
    },
    RAG_CHUNK_MAX_CHARS: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => !isNaN(parseInt(val, 10)) && parseInt(val, 10) >= 1200,
        default: '2800',
    ...
    RAG_THROTTLE_METRIC: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => ['auto', 'system', 'process'].includes(String(val)),
        default: 'auto',
    ...
    RAG_THROTTLE_MAX_DELAY_MS: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => !isNaN(parseInt(val, 10)) && parseInt(val, 10) >= 100,
        default: '5000',
    ...
    RAG_THROTTLE_SAMPLE_SIZE: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => !isNaN(parseInt(val, 10)) && parseInt(val, 10) >= 3,
        default: '8',
        message: 'Must be an integer >= 3',
    },
    RAG_THROTTLE_SLOWDOWN_FACTOR: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => !isNaN(parseFloat(val)) && parseFloat(val) > 1 && parseFloat(val) <= 3,
        default: '1.28',
        message: 'Must be a number > 1 and <= 3',
    },
    RAG_THROTTLE_SPEEDUP_FACTOR: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => !isNaN(parseFloat(val)) && parseFloat(val) > 0 && parseFloat(val) < 1,
        default: '0.92',
        message: 'Must be a number > 0 and < 1',
    },
    RAG_THROTTLE_HIGH_WATERMARK_PCT: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => !isNaN(parseFloat(val)) && parseFloat(val) >= 1 && parseFloat(val) <= 40,
        default: '8',
    ...
    RAG_THROTTLE_LOG_COOLDOWN_MS: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => !isNaN(parseInt(val, 10)) && parseInt(val, 10) >= 500,
        default: '4000',
    ...
    RAG_EXPAND_DEFAULT_LINES: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => !isNaN(parseInt(val, 10)) && parseInt(val, 10) >= 1,
        default: '40',
        message: 'Must be an integer >= 1',
    },
    RAG_EXPAND_MAX_LINES: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => !isNaN(parseInt(val, 10)) && parseInt(val, 10) >= 40,
        default: '240',
    ...
    RAG_WATCH_DEBOUNCE_MS: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => !isNaN(parseInt(val, 10)) && parseInt(val, 10) >= 250,
        default: '3000',
    ...
    MCP_GITHUB_COMMAND: {
        level: 'WARN',
        validator: (/** @type {any} */ val) => typeof val === 'string' && String(val).trim().length > 0,
        default: 'npx',
