...
function _buildTriagePrompt(job, contextPack) {
    ...
    const lines = [
        ...
        `existing_findings=${JSON.stringify(
            findings.slice(0, 8).map((findingValue) => {
                const finding = _asRecord(findingValue);
                return { title: finding['title'], severity: finding['severity'], category: finding['category'] };
            }),
        )}`.slice(0, 4000),
