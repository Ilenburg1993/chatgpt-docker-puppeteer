...
if [[ "$(stat -c '%u' "${dir}" 2>/dev/null)" != "${CURRENT_UID}" ]]; then
    ...
    if ! chown "${CURRENT_USER}:${CURRENT_USER}" "${dir}" 2>/dev/null; then
        warn "Audit: Não foi possível corrigir ownership de ${dir} (provável volume root-owned)."
    fi
fi
...
# NOTA: NWRAP não suporta comentários - arquivo deve conter apenas entradas válidas
