...
#
# v1.6.0 focus:
#   - Synchronizes with post-start.sh v2.9.0 and local-dns-cache.sh v1.6.0.
#   - Promotes .devcontainer/scripts/network/endpoints.github-copilot.tsv as the
#     canonical endpoint registry while keeping the legacy .devcontainer/network
#     path as an explicit compatibility fallback.
#   - Makes endpoint source explicit: env-override, registry, extended-default or
#     default, preventing accidental shadowing of the canonical registry.
#   - Consumes DNS runtime proof fields from local-dns-cache.sh v1.6.0, including
#     runtime_effective, resolver_effective, resolv_conf_points_to_cache and
#     previous_summary_stale, without treating old summaries as current truth.
#   - Produces clearer recommendation blockers, so "keep direct" and "run
#     comparison" are distinguishable from missing/expired artifacts.
...
Endpoint registry:
  ...
  Legacy fallback: .devcontainer/network/endpoints.github-copilot.tsv
...
DEFAULT_ENDPOINTS="https://github.com/login https://github.com/copilot https://api.github.com https://api.github.com/rate_limit https://api.github.com/user https://api.github.com/copilot_internal/v2/token https://copilot-proxy.githubusercontent.com https://origin-tracker.githubusercontent.com https://copilot-telemetry.githubusercontent.com/telemetry https://default.exp-tas.com https://api.githubcopilot.com https://api.individual.githubcopilot.com https://proxy.individual.githubcopilot.com"
EXTENDED_ENDPOINTS="https://collector.github.com https://uploads.github.com/copilot/chat/attachments/ https://user-images.githubusercontent.com https://api.business.githubcopilot.com https://proxy.business.githubcopilot.com https://api.enterprise.githubcopilot.com https://proxy.enterprise.githubcopilot.com https://copilot-reports.github.com"
ENDPOINT_REGISTRY_CANONICAL_FILE="${DEVCONTAINER_COPILOT_ENDPOINT_REGISTRY_CANONICAL_FILE:-${SCRIPT_DIR}/endpoints.github-copilot.tsv}"
readonly ENDPOINT_REGISTRY_CANONICAL_FILE
ENDPOINT_REGISTRY_LEGACY_FILE="${DEVCONTAINER_COPILOT_ENDPOINT_REGISTRY_LEGACY_FILE:-${SCRIPT_DIR}/../../network/endpoints.github-copilot.tsv}"
readonly ENDPOINT_REGISTRY_LEGACY_FILE
ENDPOINT_REGISTRY_FILE=""
ENDPOINT_REGISTRY_SOURCE="unknown"
if [[ -n "${DEVCONTAINER_COPILOT_ENDPOINT_REGISTRY_FILE:-}" ]]; then
    ENDPOINT_REGISTRY_FILE="${DEVCONTAINER_COPILOT_ENDPOINT_REGISTRY_FILE}"
    ENDPOINT_REGISTRY_SOURCE="env-file"
elif [[ -n "${DEVCONTAINER_COPILOT_ENDPOINT_REGISTRY:-}" ]]; then
    ENDPOINT_REGISTRY_FILE="${DEVCONTAINER_COPILOT_ENDPOINT_REGISTRY}"
    ENDPOINT_REGISTRY_SOURCE="env-registry"
elif [[ -r "${ENDPOINT_REGISTRY_CANONICAL_FILE}" ]]; then
    ...
    ENDPOINT_REGISTRY_SOURCE="legacy-fallback"
else
    ENDPOINT_REGISTRY_FILE="${ENDPOINT_REGISTRY_CANONICAL_FILE}"
    ENDPOINT_REGISTRY_SOURCE="canonical-missing"
fi
readonly ENDPOINT_REGISTRY_FILE ENDPOINT_REGISTRY_SOURCE
...
detect_dns_cache_state() {
    ...
    local runtime_effective resolver_effective points_to_cache resolv_managed system_uses_cache previous_stale previous_stale_reason
    ...
    # local-dns-cache.sh v1.6.0 proof fields. Older summaries will leave these
    # blank, in which case we fall back to v1.5.x semantics below.
    runtime_effective="$(summary_value_from_file "${LOCAL_DNS_SUMMARY_FILE}" runtime_effective)"
    resolver_effective="$(summary_value_from_file "${LOCAL_DNS_SUMMARY_FILE}" resolver_effective)"
    points_to_cache="$(summary_value_from_file "${LOCAL_DNS_SUMMARY_FILE}" resolv_conf_points_to_cache)"
    resolv_managed="$(summary_value_from_file "${LOCAL_DNS_SUMMARY_FILE}" resolv_conf_managed)"
    system_uses_cache="$(summary_value_from_file "${LOCAL_DNS_SUMMARY_FILE}" system_resolver_uses_cache)"
    previous_stale="$(summary_value_from_file "${LOCAL_DNS_SUMMARY_FILE}" previous_summary_stale)"
    previous_stale_reason="$(summary_value_from_file "${LOCAL_DNS_SUMMARY_FILE}" previous_summary_stale_reason)"
    ...
    DNS_CACHE_RUNTIME_EFFECTIVE="${runtime_effective:-unknown}"
    DNS_CACHE_RESOLVER_EFFECTIVE="${resolver_effective:-unknown}"
    DNS_CACHE_RESOLV_CONF_POINTS_TO_CACHE="${points_to_cache:-unknown}"
    DNS_CACHE_RESOLV_CONF_MANAGED="${resolv_managed:-unknown}"
    DNS_CACHE_SYSTEM_RESOLVER_USES_CACHE="${system_uses_cache:-unknown}"
    DNS_CACHE_PREVIOUS_SUMMARY_STALE="${previous_stale:-unknown}"
    DNS_CACHE_PREVIOUS_SUMMARY_STALE_REASON="${previous_stale_reason:-unknown}"
    ...
    if [[ "${DNS_CACHE_RESOLV_CONF_DRIFT}" == "true" ]]; then
        ...
        DNS_CACHE_STATUS="runtime-only"
    elif [[ "${local_status}" == "ok" && -z "${runtime_effective}" && ("${local_resolv}" == "updated" || "${local_resolv_health}" == *"points-to-cache"* || "${active_by_resolv}" == "true") ]]; then
        DNS_CACHE_STATUS="ok-legacy-summary"
    ...
    elif [[ "${DNS_CACHE_STATUS}" == "runtime-only" ]]; then
        DNS_CACHE_EFFECTIVE="runtime-only"
...
doctor_action() {
    ...
    if validate_endpoint_registry; then
        log_ok "doctor: endpoint registry OK: ${ENDPOINT_REGISTRY_FILE}; source=${ENDPOINT_REGISTRY_SOURCE}; rows=${ENDPOINT_REGISTRY_ROWS}; endpoint_source=${ENDPOINT_SOURCE}."
    else
...
validate_manager_contract() {
    if [[ "${ENDPOINT_REGISTRY_SOURCE}" == "legacy-fallback" ]]; then
        append_report "contract_warning=endpoint_registry_legacy_fallback canonical=${ENDPOINT_REGISTRY_CANONICAL_FILE} legacy=${ENDPOINT_REGISTRY_LEGACY_FILE}"
    elif [[ "${ENDPOINT_REGISTRY_SOURCE}" == "canonical-missing" && "${USE_ENDPOINT_REGISTRY}" == "true" ]]; then
        append_report "contract_warning=endpoint_registry_canonical_missing expected=${ENDPOINT_REGISTRY_CANONICAL_FILE}"
    fi
    if [[ "${ENDPOINT_SOURCE}" == "env-override" && "${ALLOW_CUSTOM_ENDPOINTS}" != "true" ]]; then
        append_report "contract_notice=endpoint_env_override_present_allowlist_enforced"
    fi
    if [[ "${GITHUB_API_HOST}" != "api.github.com" && "${ALLOW_CUSTOM_GITHUB_API_HOST}" != "true" ]]; then
...
generate_manager_recommendation() {
    ...
    if [[ "${route_artifact_state}" == "missing" || "${route_artifact_state}" == "expired" ]]; then
        recommendation_blockers="$(join_recommendation "${recommendation_blockers}" "route-artifact-${route_artifact_state}")"
    ...
    if [[ "${proxy_artifact_state}" == "missing" || "${proxy_artifact_state}" == "expired" ]]; then
        recommendation_blockers="$(join_recommendation "${recommendation_blockers}" "proxy-artifact-${proxy_artifact_state}")"
    ...
    case "${proxy_action}" in
        ...
        keep-direct)
            ...
            ;;
        unknown)
            if [[ "${PLANE_COPILOT_TRANSPORT_STATUS}" == "failed" || "${PLANE_COPILOT_TRANSPORT_STATUS}" == "degraded" || "${PRIMARY_BOTTLENECK}" == "tcp-bound" || "${PRIMARY_BOTTLENECK}" == "tls-bound" || "${PRIMARY_BOTTLENECK}" == "server-bound" ]]; then
                action="run-compare-transports"
                transport="direct"
                confidence="low"
                reason="proxy-comparison-missing-and-current-transport-needs-evidence"
            else
                action="keep-direct"
                transport="direct"
                confidence="low"
                reason="current-transport-ok;proxy-comparison-missing"
            fi
            ;;
    esac
    ...
    MANAGER_RECOMMENDATION_BLOCKERS="${recommendation_blockers}"
