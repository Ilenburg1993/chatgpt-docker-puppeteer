...
try {
    ...
    await Promise.race([
        ...
        new Promise((_, reject) =>
            setTimeout(() => reject(new Error(`SSOT init timeout após ${SSOT_INIT_TIMEOUT}ms`)), SSOT_INIT_TIMEOUT)
        )
    ]);
} catch (err) {
...
**Prioridade:** P2
**Arquivo:** `src/main.js:787, 841` (2 localizações)
...
**Tempo Total Sprint 2:** ~1.5 horas
**Próxima Revisão:** Após smoke tests em ambiente de testes
