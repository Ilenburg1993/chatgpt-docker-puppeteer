...
export function buildQualityExecutionPlan(options) {
  ...
  const forceFull = qualityMode === 'full' || (qualityMode === 'smart' && profile === 'quick' && impact.hasHighRiskConfig);
  ...
  const nodeCheckMode = forceFull
    ? (jsFiles.length > 0 ? 'changed-only' : 'skip')
    : changedOnly
      ? (jsFiles.length > 0 ? 'changed-only' : 'skip')
      : 'skip';

  const lintMode = impact.onlyDocs && !impact.hasHighRiskConfig
    ? 'skip'
    : forceFull
      ? 'full'
      : changedOnly
        ? ((impact.hasCode || impact.hasHighRiskConfig) ? 'changed-only' : 'skip')
        : 'skip';

  const typecheckNodeMode = !impact.hasNodeTypeImpact
    ? 'skip'
    : 'full';

  const typecheckBrowserMode = !impact.hasBrowserTypeImpact
    ? 'skip'
    : 'full';

  const prettierMode = options.qualityPrettier === false
    ? 'skip'
  ...
  const jsdocDeltaMode = options.qualityJsdoc === false
    ? 'skip'
    : profile === 'quick'
      ? (jsdocDeltaFiles.length > 0 ? 'changed-only' : 'skip')
      : 'skip';

  const jsdocFullMode = options.qualityJsdoc === false
    ? 'skip'
    : profile === 'quick'
      ? 'skip'
      : 'full';

  const tsIgnoreScanMode = profile === 'quick'
    ? (impact.changed.length > 0 ? 'changed-only' : 'skip')
    : 'full';
