...
function validateContract(contract, index, sourcePath) {
    ...
    if (!contract.description || typeof contract.description !== 'string') errors.push(`${ptr}: description obrigatório`);
    ...
    if (!Array.isArray(contract.test_recipe) || contract.test_recipe.length === 0) errors.push(`${ptr}: test_recipe obrigatório`);
...
export function loadContractRegistry(options = {}) {
    ...
    for (const relPath of allowlistPaths) {
        ...
        for (const [key, value] of Object.entries(entries)) {
            allowlists[allowlistId][key] = Array.isArray(value) ? value.map(item => String(item).replace(/\\/g, '/')) : [];
        }
    }
