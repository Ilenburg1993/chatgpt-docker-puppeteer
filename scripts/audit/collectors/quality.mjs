...
function makeCachePath(cacheDir, stepKey, cacheInput) {
  const hash = sha256(stableJson(cacheInput));
 ...
 * @returns {RawFinding}
 */
function finding(sourceTool, file, line, evidence, rule, severity, type, impact, rootCause, suggestedPatch, testStrategy, meta = {}) {
  return {
...
export function parseTsIgnoreFindings(stdout, scopeFiles) {
  ...
  for (const line of String(stdout || '').split(/\r?\n/)) {
    const m = line.match(/^(.+?):(\d+):(.*@ts-ignore.*)$/);
    if (!m) continue;
    ...
    findings.push(finding(
      ...
      'P1',
      'falha de contrato',
      'Uso de `@ts-ignore` mascara erros de tipagem e reduz robustez do código.',
      ...
      'Executar scan de `@ts-ignore` e `typecheck:full`.',
...
export async function collectQualityFindings(options) {
  ...
  const entrypointSmokeTask = runPlanned('quality.entrypoint_import_smoke', plan.steps.entrypoint_import_smoke.mode, async () => {
    await exec('quality.entrypoint_import_smoke', 'node', ['-e', 'process.stdout.write(\"quality-entrypoint-smoke\")'], {
      timeoutMs: 10000,
      acceptExitCodes: [0],
      env: { NO_COLOR: undefined },
    });
  ...
  await runPlanned('quality.lint', plan.steps.lint.mode, async () => {
    const args = plan.steps.lint.mode === 'changed-only' && (plan.steps.lint.files || []).length > 0
    ...
    const { value: res } = await runCached(
      ...
      }),
      {
        ...
        configs: fileSigs(['package.json', 'eslint.config.js', 'eslint.config.mjs', '.eslintrc', 'tsconfig.json', 'tsconfig.browser.json', 'jsconfig.json']),
      },
      async () => exec('quality.lint', 'npx', args, {
        timeoutMs: 300000,
    ...
    if (!res.ok && lintFindings.length === 0) {
      warnings.push({ source: 'quality:lint', message: (res.stderr || res.stdout || 'eslint failed sem parsing').slice(0, 4000) });
    }
  ...
  await runPlanned('quality.typecheck_node', plan.steps.typecheck_node.mode, async () => {
    const { value: res } = await runCached(
      ...
      () => exec('quality.typecheck_node', 'node', ['-e', 'process.stdout.write(\"quality-typecheck-node-cache-hit\")'], {
        timeoutMs: 10000, acceptExitCodes: [0], env: { NO_COLOR: undefined },
      }),
      {
        profile: options.profile,
      ...
      async () => exec('quality.typecheck_node', 'npm', ['run', '-s', 'typecheck:node'], {
        timeoutMs: 300000,
        acceptExitCodes: [0, 1, 2],
        env: { NO_COLOR: undefined },
      })
    ...
    if (!res.ok && typeFindings.length === 0) {
      warnings.push({ source: 'quality:typecheck_node', message: (res.stderr || res.stdout || 'typecheck:node failed sem parsing').slice(0, 4000) });
    }
  ...
  await runPlanned('quality.typecheck_browser', plan.steps.typecheck_browser.mode, async () => {
    const { value: res } = await runCached(
      ...
      () => exec('quality.typecheck_browser', 'node', ['-e', 'process.stdout.write(\"quality-typecheck-browser-cache-hit\")'], {
        timeoutMs: 10000, acceptExitCodes: [0], env: { NO_COLOR: undefined },
      }),
      {
        profile: options.profile,
      ...
      async () => exec('quality.typecheck_browser', 'npm', ['run', '-s', 'typecheck:browser'], {
        timeoutMs: 300000,
        acceptExitCodes: [0, 1, 2],
        env: { NO_COLOR: undefined },
      })
    ...
    if (!res.ok && typeFindings.length === 0) {
      warnings.push({ source: 'quality:typecheck_browser', message: (res.stderr || res.stdout || 'typecheck:browser failed sem parsing').slice(0, 4000) });
    }
    return typeFindings.length;
  });

  await runPlanned('quality.prettier_check', plan.steps.prettier_check.mode, async () => {
    const changedMode = plan.steps.prettier_check.mode === 'changed-only' && (plan.steps.prettier_check.files || []).length > 0;
    ...
    const { value: res } = await runCached(
      ...
      }),
      {
        ...
        configs: fileSigs(['package.json', '.prettierrc', '.prettierrc.json', 'prettier.config.js', 'prettier.config.mjs']),
      },
      async () => changedMode
    ...
    if (!res.ok && prettierFindings.length === 0) {
      warnings.push({ source: 'quality:prettier', message: (res.stderr || res.stdout || 'prettier --check failed sem parsing').slice(0, 4000) });
    }
  ...
  const jsdocDeltaTask = runPlanned('quality.jsdoc_delta', plan.steps.jsdoc_delta.mode, async () => {
    ...
    const { value: res } = await runCached(
      ...
      }),
      {
        ...
        engine: fileSigs(['scripts/analysis/jsdoc_coverage_engine.mjs', 'scripts/analysis/jsdoc_coverage_cli.mjs']),
      },
      async () => exec('quality.jsdoc_delta', 'node', [
  ...
  const tsIgnoreScanTask = runPlanned('quality.ts_ignore_scan', plan.steps.ts_ignore_scan.mode, async () => {
    ...
    const { value: res } = await runCached(
      ...
      async () => exec('quality.ts_ignore_scan', 'rg', ['-n', '@ts-ignore', 'src', 'scripts', 'tests', '--glob', '!**/dist/**'], {
        timeoutMs: 30000,
        acceptExitCodes: [0, 1],
        env: { NO_COLOR: undefined },
      })
  ...
  await runPlanned('quality.jsdoc_full', plan.steps.jsdoc_full.mode, async () => {
    const { value: res } = await runCached(
      ...
      async () => exec('quality.jsdoc_full', 'node', [
        'scripts/analysis/jsdoc_coverage_cli.mjs',
        '--scope',
        'full',
        '--format',
