...
function collectExportsFromSourceFile(sf) {
  ...
  for (const stmt of sf.statements) {
    if (ts.isFunctionDeclaration(stmt) && isExportedStatement(stmt) && stmt.name) {
      out.push(buildAssessment(stmt, sf, stmt.name.text, 'function'));
      continue;
    }

    if (ts.isClassDeclaration(stmt) && isExportedStatement(stmt) && stmt.name) {
      out.push(buildAssessment(stmt, sf, stmt.name.text, 'class'));
      continue;
    }
...
export function analyzeJSDocCoverage(options) {
  ...
  for (const item of Object.values(byPathPrefix)) {
    item.coverage_pct = item.exports_total > 0 ? Number(((item.exports_with_jsdoc / item.exports_total) * 100).toFixed(1)) : 100;
  }
