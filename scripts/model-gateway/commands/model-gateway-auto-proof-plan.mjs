...
if (argSet.has('--help') || argSet.has('-h')) {
    process.stdout
        .write(`Usage: node scripts/model-gateway/commands/model-gateway-auto-proof-plan.mjs [--json] [--profile ID] [--fallback-profiles a,b] [--selection-policy metadata_first|prefer_runtime_proved|require_runtime_proof] [--temporary-failure-cooldown-ms N] [--limit N] [--timeout-ms N]
...
if (argSet.has('--json')) {
    ...
    for (const row of commandRows.slice(0, limit)) {
        process.stdout.write(
            `  ${row.command}  # profile=${row.profileId} reasons=${row.reasons.slice(0, 3).join('+') || '-'}\n`,
        );
    }
    if (commandRows.length === 0) {
        process.stdout.write(
            '  No runtime proof commands are currently needed or derivable from top blocked alternatives.\n',
        );
    }
}
