...
function runPersistentRedactionWorker(/** @type {'catalog' | 'sqlite'} */ mode, /** @type {number} */ maxRowsPerTable) {
...
export async function buildModelGatewayLiveReadiness(options = {}) {
    ...
    const secretAuditValues = timedSync('secretAuditEnvCollection', () =>
        collectModelGatewaySecretAuditEnvValues(process.env),
    );
    const secretRegistry = createEnvSecretRegistry();
    const selectionEnvironmentIdentity = timedSync('selectionEnvironmentIdentity', () =>
        readinessEnvironmentIdentity(process.env),
    );
...
if (isDirectCli) {
    if (argSet.has('--help') || argSet.has('-h')) {
        ...
        if (json) {
            ...
            summary['livePlan'].commands.forEach((command, index) =>
