...
run_post_tool_use_hook() {
    ...
    if [ -f "$CTX_FILE" ]; then
        if [ "$TOOL_NAME" = "vscode_askQuestions" ] && [ -n "$TOOL_RESPONSE" ]; then
            ...
            if [ "$ASK_TEMPLATE_F" != "true" ]; then
                jq -cn \
                    ...
                    '{
                        ...
                        timestamp: $ts,
                        tool_use_id: $tool_use_id,
                        message: "Resposta de continuidade registrada — fechamento de TURN é válido; Template F permanece restrito a SESSION Close"
                    }' >> "$AUDIT_FILE"
            fi
            ...
            if [ "$ASK_TEMPLATE_F" != "true" ]; then
                _CONT_MSG="ℹ️ Resposta de continuidade registrada. Fechamento de TURN é válido neste ponto; Template F é apenas para SESSION Close."
                _CONT_CTX="Se você continuar com ferramentas de trabalho após este ponto, finalize novamente com vscode_askQuestions no fim do ciclo. Template F só em escalonamento explícito para encerrar SESSION."
                if [ "$ASK_CONTINUATION_UNCLEAR" = "true" ]; then
                    _CONT_MSG="⚠️ Resposta ambígua detectada em askQuestions de continuidade: auto-auditoria obrigatória ativada para este TURN."
                    _CONT_CTX="Inicie auditoria do escopo atual (leitura/busca/diagnóstico) antes de novos passos de edição; após continuar o trabalho, finalize com novo vscode_askQuestions."
                fi
            ...
            if command -v mark_turn_todo_created_true > /dev/null 2>&1; then
                mark_turn_todo_created_true "$TIMESTAMP" > /dev/null 2>&1 || true
            else
