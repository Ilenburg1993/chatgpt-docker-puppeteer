...
- Se `rag:health.ok=false`, seguir em modo lexical/fallback e marcar risco explicitamente no
  tracker.
- Toda rodada deve gerar ou atualizar snapshot em `DOCUMENTAÇÃO/BUGS/rodadas/`.
- Em tuning de performance do quick, comparar pares de runs (`cache miss` vs `cache hit`) e
  registrar deltas de `duration_ms_total` + `quality_execution.cache`.
