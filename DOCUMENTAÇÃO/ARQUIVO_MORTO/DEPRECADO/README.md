# DEPRECADO

**Propósito**: concentrar material explicitamente deprecated, preservado apenas por
rastreabilidade.  
**Status documental**: Histórico.  
**Público**: engenharia, manutenção e auditoria.  
**Última atualização**: 28 de fevereiro de 2026.

## Natureza histórica

Esta árvore reúne documentação já marcada como deprecated e retirada do fluxo canônico.

## Subárvores locais

- [DEPRECATIONS/README.md](./DEPRECATIONS/README.md)

## O que não é canônico

- nenhum documento aqui deve orientar novas mudanças sem revalidação explícita.

## Links relacionados

- Índice do arquivo morto: [../README.md](../README.md)
