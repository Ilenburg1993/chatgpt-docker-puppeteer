# ANALISE_LEGADA

**Propósito**: organizar análises legadas reclassificadas por tipo, preservando contexto histórico
sem competir com a documentação viva.  
**Status documental**: Histórico.  
**Público**: engenharia, auditoria e agentes de IA.  
**Última atualização**: 28 de fevereiro de 2026.

## Natureza histórica

Esta árvore agrupa análises antigas que foram achatadas e reorganizadas para consulta mais legível.

## Subárvores locais

- [ROOT/README.md](./ROOT/README.md)
- [CODE_SUMMARY/README.md](./CODE_SUMMARY/README.md)
- [ISSUES/README.md](./ISSUES/README.md)
- [LEGACY/README.md](./LEGACY/README.md)
- [NOTIFICATIONS/README.md](./NOTIFICATIONS/README.md)
- [ROTATION_SCRIPTS/README.md](./ROTATION_SCRIPTS/README.md)

## O que não é canônico

- esta árvore é histórica;
- os materiais daqui só devem ser usados como contexto ou rastreabilidade.

## Links relacionados

- Índice do arquivo morto: [../README.md](../README.md)
