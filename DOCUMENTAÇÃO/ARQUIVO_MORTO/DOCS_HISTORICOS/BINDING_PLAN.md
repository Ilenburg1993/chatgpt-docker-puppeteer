...
1. Escanear código estático em src/ para ocorrências de binding explícito em `127.0.0.1` ou
   `localhost`.
2. Executar checagem runtime para portas canônicas (3000,3001,3002,3008,9100,9224).
3. Corrigir código para usar `0.0.0.0` quando apropriado e adicionar logs de boot que exibam o
   endereço de binding.
...
- Se forem encontradas ocorrências estáticas, revisar manualmente ou rodar
  `node scripts/fix-bindings.js ./src --apply`.
- Adicionar target no Makefile / job no CI para executar `./scripts/check-all-bindings.sh` antes de
  testes.
