# SUPERSEDED

**Propósito**: preservar variantes substituídas que foram mantidas por rastreabilidade, comparação e
auditoria.  
**Status documental**: Histórico.  
**Público**: engenharia, manutenção e auditoria.  
**Última atualização**: 28 de fevereiro de 2026.

## Natureza histórica

Esta pasta guarda documentos e variantes que já foram substituídos por versões mais novas ou por
documentação canônica consolidada.

## O que não é canônico

- as versões daqui não devem orientar novas decisões;
- servem apenas para comparação histórica e rastreabilidade.

## Links relacionados

- Índice do arquivo morto: [../README.md](../README.md)
