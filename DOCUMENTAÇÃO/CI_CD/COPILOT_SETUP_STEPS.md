...
## Visão geral da arquitetura de cache (v3.0)

O workflow usa **cache multicamadas** para reduzir o tempo de setup de ~10 minutos (cold) para
~2–3 minutos (warm hit):
...
Camada 2: actions/cache@v5 — dashboard-ui node_modules
  └── src/dashboard-ui/node_modules
  └── Chave: runner.os + hash de src/dashboard-ui/package-lock.json
  └── Miss: npm ci no dashboard-ui (~40s)

Camada 3: actions/cache@v5 — ferramentas globais npm
  └── ~/.npm-global-copilot (prefixo isolado)
...
Camada 4: actions/cache@v5 — packages Python
  └── ~/.cache/pip
...
O workflow espelha **exatamente** as versões definidas como `ARG` no `.devcontainer/Dockerfile`.
Ao atualizar o Dockerfile, **atualize também as variáveis `env:` do workflow**.
...
**Total estimado (cold)**: ~8–10 min  
**Total estimado (warm)**: ~3–4 min
