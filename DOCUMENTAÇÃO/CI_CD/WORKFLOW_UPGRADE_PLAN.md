...
- Maioria dos `actions/github-script` sem `github-token` explícito (usa implicitamente `GITHUB_TOKEN`)
...
3. **`raven-actions/actionlint@v2.1.1` → `@v2.1.2`** — `ci.yml` + `scripts/ci/validate-workflows.mjs`
...
Isso garante que, quando o secret `GH_PAT` estiver configurado, ele será utilizado
(maior rate limit, mais escopos); caso contrário, usa o `GITHUB_TOKEN` padrão.
...
Os workflows usarão automaticamente o PAT quando disponível, voltando ao `GITHUB_TOKEN` se não estiver configurado.
