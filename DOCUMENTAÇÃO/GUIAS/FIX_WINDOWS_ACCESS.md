...
**Causa**: Docker Desktop (WSL2) não roteia tráfego do Windows diretamente para IPs internos do
container (172.17.x.x).
...
O Vite já está configurado para escutar em `0.0.0.0:5173` (todas as interfaces), então o port
forwarding funciona automaticamente.
...
**Se retornar "False"**: → Docker Desktop não está fazendo port forward
...
**Versão**: 1.1 **Data**: 2026-02-04 **Status**: ✅ Networking Fix - Use localhost ao invés de IP
