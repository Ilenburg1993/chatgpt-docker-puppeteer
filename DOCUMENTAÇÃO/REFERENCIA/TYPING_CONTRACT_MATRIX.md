...
This matrix maps each canonical typing/JSDoc surface to its contract, validator, CI gate, and
owner role.
...
| Aggregate typing audit | CI, maintainers | Structured audit JSON plus console contract | `scripts/analysis/typing_hardening_audit.mjs` | Script self-validation plus strict thresholds | `analyze:typing`, `analyze:typing:public` | Typing audit maintainers |
| Local tsserver wrapper envelope | LSP tooling, MCP-facing adapter code | Wrapper JSON Schema plus `ts.server.protocol` mapping | `schemas/typing/tsserver-tool-contract.schema.json` and `src/integration/lsp/tsserver-daemon.mjs` | `tsserver_contract_audit.mjs` | `analyze:tsserver-contract`, `check:schemas:typing` | LSP wrapper maintainers |
...
| Strict public lane | Public contract anchors | Strict `tsc` lane | `tsconfig.strict.public.json` | `tsc -p tsconfig.strict.public.json` | `typecheck:strict:public`, `typecheck:strict:all` | Strict lane maintainers |
| Strict workspace lanes | Maintainers, CI | Strict solution refs | `tsconfig.strict.json` and `tsconfig.strict.*.json` | `tsc` per lane | `typecheck:strict:all` | Strict lane maintainers |
