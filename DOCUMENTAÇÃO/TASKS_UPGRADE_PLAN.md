...
2. **Escrever novo tasks.json** contendo:
   ...
   - Tarefas de teste (fast, integration, all).
...
3. **Remover a velha tasks.json** e substituir pelo novo (mantendo backup `tasks.legacy.json` se
   ...
   garantir que nada quebre. 7. **Rodar `npm run test:fast` / `make test-fast`** através das tasks
   para confirmar.
...
```json
{
  ...
  "tasks": [
    ...
    { "label": "format", "type": "shell", "command": "make format-code", "group": "build" },

    { "label": "test:fast", "type": "shell", "command": "make test-fast", "group": "test" },
    {
    ...
    { "label": "pm2:monitor", "type": "shell", "command": "make pm2-monit", "isBackground": true },

    { "label": "logs:follow", "type": "shell", "command": "make logs", "isBackground": true },
    { "label": "logs:watch", "type": "shell", "command": "make watch", "isBackground": true },

    { "label": "queue:status", "type": "shell", "command": "make queue" },
    {
      "label": "queue:watch",
      "type": "shell",
    ...
    { "label": "backup", "type": "shell", "command": "make backup" },

    { "label": "info", "type": "shell", "command": "make info" },
    { "label": "version", "type": "shell", "command": "make version" },
    { "label": "check-deps", "type": "shell", "command": "make check-deps" },

    {
      "label": "validate-all",
      "dependsOn": ["lint", "test:fast", "health:core", "git:changed"],
