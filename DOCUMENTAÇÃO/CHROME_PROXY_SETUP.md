...
**Versão**: 1.0 **Data**: 2026-01-30 **Status**: ✅ Solução Arquitetural Permanente

---

## 📋 Visão Geral

Este documento descreve a configuração e uso do **Chrome Proxy Service**, uma solução arquitetural
para conectar Puppeteer (rodando em container Docker) ao Chrome (rodando no Windows Host).

### Problema Resolvido

Container Docker não consegue conectar ao Chrome no Windows porque o `webSocketDebuggerUrl`
retornado pelo Chrome contém `ws://localhost:9224/...` ou `ws://127.0.0.1:9224/...`, que não são
acessíveis do container.
...
- [Chrome DevTools Protocol](https://chromedevtools.github.io/devtools-protocol/) - Especificação
  oficial
...
Chrome ignora ou sobrescreve esta flag em muitos ambientes Windows, continuando a escutar apenas em
`127.0.0.1`. O proxy é mais confiável.
...
Sim! O proxy é Node.js puro e funciona em qualquer plataforma. A documentação foca em Windows porque
é o cenário mais complexo (Docker + Windows Host).
...
**Última atualização**: 2026-01-30 **Versão**: 1.0 **Manutenção**: Atualizar quando mudanças no
proxy forem feitas
