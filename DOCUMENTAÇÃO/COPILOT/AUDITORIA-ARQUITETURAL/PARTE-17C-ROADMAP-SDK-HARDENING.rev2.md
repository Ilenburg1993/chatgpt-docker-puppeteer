...
**Escopo**: Implementação completa do SDK layer + integração agent/observability/bridges — 10 faixas, 58 fases
**Baseado em**: PARTE-17A rev.2 (situação atual) + PARTE-17B rev.2 (situação ideal)
...
- **Ação**: Remover chamada `loadCustomTools()` no final do módulo. Exportar `initCustomTools()` async para bootstrap
...
- **Ação**: Atualizar `SessionCreateOptions` e `SessionResumeOptions` JSDoc para incluir todos os campos do SDK:
  `sessionId`, `clientName`, `configDir`, `agent`, `skillDirectories`, `disabledSkills`,
  `availableTools`, `excludedTools`, `onEvent`, `onElicitationRequest`, `commands`
- **Testes novos**: 5

### F009 — Alinhar tipos CopilotClientOptions com SDK 0.2.1
- **Arquivo**: `sdk/client.js`
- **Ação**: Refatorar `buildClientOptions()` para aceitar e passar todos os campos:
  `cliArgs`, `cwd`, `port`, `useStdio`, `isChildProcess`, `logLevel`, `env`,
  `githubToken`, `useLoggedInUser`, `onListModels`, `onGetTraceContext`, `telemetry`
- **Testes novos**: 5
...
- **Ação**: Substituir typedefs locais por `@typedef {import('@github/copilot-sdk').X}` onde X existe no SDK
...
- **Arquivo**: `sdk/session.js`
- **Ação**: Adicionar campo `provider` em `buildSessionConfig()`, validar tipo antes de passar ao SDK
- **Testes novos**: 4
...
- **Ação**: tools-state como orchestrator — converte allowlist → `availableTools`, denylist → `excludedTools`
- **Testes novos**: 4

### F023 — SessionConfig full passthrough
- **Arquivo**: `sdk/session.js`
- **Ação**: buildSessionConfig aceitar e propagar: `sessionId`, `clientName`, `configDir`, `agent`, `skillDirectories`, `disabledSkills`, `onEvent`
- **Testes novos**: 6
...
- **Ação**: Executar `npm run lint && npm run format:check && npm run test:unit && npm run typecheck:node`
...
- **Arquivo**: `sdk/session.js`
- **Ação**: Adicionar suporte a `availableTools`, `excludedTools`, `skillDirectories`, `customAgents`, `infiniteSessions`, `workingDirectory`, `onPermissionRequest`, `onUserInputRequest`
...
- **Ação**: `buildSessionConfig()` consulta `getCurrentToolsState()` internamente; merge com allowlist/denylist explícitos caso fornecidos
...
- **Ação**: Substituir construção manual de config por `buildSessionConfig({...overrides})`. Verificar paridade total com comportamento anterior
...
- **Ação**: Adicionar `@typedef` re-exports para: `Tool`, `ToolHandler`, `ToolInvocation`, `PermissionHandler`, `PermissionRequest`, `UserInputRequest`, `MessageOptions`, `SessionConfig`, `SystemMessageConfig`, `InfiniteSessionConfig`, `MCPServerConfig`, `CustomAgentConfig`, `SessionHooks`
...
- **Ação**: Substituir imports por `#copilot/sdk`. Exceção: `sdk/` internos que naturalmente importam do pacote
- **Referência**: ARCH-INT-03
...
- **Ação**: Teste end-to-end que verifica: `buildSessionConfig(agentOverrides)` → config contém todos os campos → mock de `resumeOrCreate()` recebe config completo
- **Referência**: Validação ARCH-INT-01/02/03
- **Testes**: 6 — cenários normal, customize mode, tools override, skillDirs, agents, infiniteSessions
...
- **Ação**: Importar `DEFAULT_PERSIST_TYPES` e comparar com lista canônica de events conhecidos (extraída dos .d.ts do SDK). Falhar se há divergência
...
- **Ação**: Importar `TelemetryConfig` type do SDK (via JSDoc `@type`); garantir que `buildTelemetryConfig()` retorna tipo compatível. Adicionar JSDoc bridge
...
- **Ação**: Adicionar handler para `session.capabilities` change events (quando SDK 0.2.1 os emitir). Registrar em métricas e alerting
...
- **Ação**: Adicionar `recordElicitation(durationMs, type, outcome)` com histograma rolling. Alimentar a partir do event-collector
...
- **Ação**: 1 test que mocka uma session SDK com eventos de tool/token/error/elicitation e verifica que event-collector + metrics + tool-stats registram tudo corretamente
...
- **Ação**: Comparar `EVENT_MAP` keys com lista canônica de agent events. Falhar se eventos novos sem mapeamento
...
- **Ação**: Adicionar mapeamentos faltantes para eventos 0.2.1: `elicitation.requested`, `elicitation.completed`, `command.executed`, `session.capabilities_changed`, `session.ui_action`
- **Referência**: BRG-GAP-01
- **Testes**: 3 — cada novo mapping verificado

### F056 — MCP Config full passthrough
- **Arquivo**: `bridges/mcp-tool-bridge.js`
- **Ação**: Aceitar todos os campos de `MCPServerConfig` do SDK (`oauthOptions`, `environmentVariables`, `timeout`, `serverFilter`)
...
- **Ação**: Emitir eventos `mcp.reconnect.started` / `mcp.reconnect.completed` / `mcp.reconnect.failed` capturados pelo event-collector
...
- **Ação**: Teste que verifica: agent EventEmitter → nerv-bridge → NERV envelope para todos os event types mapeados
