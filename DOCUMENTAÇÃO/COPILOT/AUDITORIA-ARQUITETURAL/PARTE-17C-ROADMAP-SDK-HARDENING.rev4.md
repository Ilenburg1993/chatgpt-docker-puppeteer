...
**Data**: 2026-03-20 (rev.4 — roadmap expandido com cobertura completa do SDK) **Base**: PARTE-17A
rev.4 (análise) + PARTE-17B rev.4 (proposta) **Autor**: Auditoria automatizada PARTE-17

---

## Sumário Executivo

O roadmap rev.4 expande de **12 faixas / 67 fases / ~250 testes** (rev.3) para **22 faixas / ~130+
fases / ~500+ testes**. A expansão cobre todas as features do SDK identificadas na análise rev.4,
incluindo 17 subsistemas RPC, 70+ event types tipados, novas features (health checks, quota
monitoring, mode/model switching, plan management, compaction, elicitation, etc.), e infraestrutura
de extensibilidade para features experimentais.
...
**Resolve**: P10 (types duplicados) + P4 (hooks types paralelos) **Cria**: `sdk/types.js`,
`sdk/constants.js`
...
**Resolve**: P5 (defineTool bypass), P6 (approveAll bypass) **Cria**: `sdk/tools.js`,
`sdk/permissions.js`
...
**Resolve**: P9 (SYSTEM_PROMPT_SECTIONS bypass), P13 (customize mode ausente) **Cria**:
`sdk/system-message.js`
...
**Resolve**: P1 (3 config paths) **Cria**: `sdk/config.js`
...
**Resolve**: P2 (dual registry), P7 (CopilotClient bypass), P14 (no health check), P15 (no auth
check) **Refatora**: `sdk/client.js`
...
**Resolve**: P17 (abort ausente), expansão de métodos **Refatora**: `sdk/session.js`
...
**Resolve**: P11 (RPC não expostos — parte 1: estáveis) **Cria**: `sdk/rpc.js` (parte 1)
...
**Tests**: ~44 | **Commit**:
`feat(sdk): F7 — RPC facade core subsystems (model, mode, plan, workspace)`

---

### Faixa 8 — RPC Facade: Advanced Subsystems 🟡

**Resolve**: P11 (RPC não expostos — parte 2) **Expande**: `sdk/rpc.js`
...
**Tests**: ~34 | **Commit**:
`feat(sdk): F8 — RPC facade advanced subsystems (shell, ui, compaction)`

---

### Faixa 9 — Server RPC Facade 🟡

**Resolve**: P16 (quota não monitorada), P14 (no health check) **Cria**: `sdk/health.js`, expande
`sdk/rpc.js` com `createServerRpc()`
...
**Resolve**: P12 (70+ events sem tipagem) **Cria**: `sdk/events.js`
...
**Resolve**: gap de ProviderConfig não wrapped **Cria**: `sdk/provider.js`
...
_Documento gerado pela auditoria PARTE-17, rev.4. Roadmap expandido de 12→22 faixas, 67→~125 fases,
~250→~574 testes. Base: API Surface completa do SDK (9 arquivos de declaração, 4.498 linhas).
