...
> **Documento**: PARTE-24K-ROADMAP-ULTRA-DETALHADO.md **Versão**: 1.2 **Data**: 2026-04-13
> (atualizado) **Escopo**: Roadmap de implementação cirúrgico e completo — arquivo de referência
> máster para toda a reestruturação do `src/copilot`. Substitui PARTE-24J como roadmap canônico.
...
- **GET simples**: /context, /quota, /pr-budget, /config, /config/skills, /config/tools, /errors,
  /tool-stats, /history, /system/reset
- **GET com query params**: /audit, /sessions, /sessions/:id/turns, /memory, /gh/issues, /gh/prs,
  /gh/ci, /git/status, /git/log
- **SSE**: GET /events (custom, rate-limited)
- **PUT**: /config/infinite-session, /config/skills, /config/tools
- **POST**: /config/tools/custom, /memory, /pipeline, /inject, /dialog/pause, /dialog/resume,
  /handoff, /handoff/:id/accept, /handoff/:id/reject
...
- `system-metrics.js` — errors, tool-stats, history, audit, pr-budget, git/status, git/log,
  gh/issues, gh/prs, gh/ci
...
> **Objetivo**: Criar `src/copilot/server/` com estrutura completa, mas **sem inicializar ainda**. O
> sistema deve funcionar **exatamente igual** ao estado anterior após esta onda.
>
...
export function createRateLimiter(max, windowMs) {
  /* mesma impl de terminal/server.js */
}
  ...
  return (req, res, next) => {
    ...
    if (!result.allowed) {
      ...
      res
        ...
        .json({
          ok: false,
          error: errorMsg ?? `Rate limit excedido. Tente em ${result.resetIn}s.`,
        });
      return;
    }
    next();
...
**Acceptance**: Arquivo existe, exporta `createRateLimiter`, `createDefaultRateLimiters`,
`rateLimitMiddleware`.
...
**Nota sobre Express**: verificar `package.json` se Express está listado como dependência. Se não
estiver, adicionar com justificativa: "copilot/server precisa de Express para middleware, routers e
error handling estruturado". Socket.io também requer.

---

### L54.7 — Criar `src/copilot/server/index.js` (stub)

**Stub**: Arquivo existe, exporta `createCopilotServer` como stub que ainda não faz nada (não inicia
server). Será completado na Onda 3.2.
 ...
 *
 * NOTA: Implementação completa adicionada na Onda 3.2 (Socket.IO). Esta versão bootstrap apenas monta Express + HTTP.
 *
...
**Commit da Onda 3.0**:
`refactor(onda3.0): L54.1-L54.7 — scaffold copilot/server/, middleware Express`
...
> duplicados.
>
> **Invariante**: `terminal/server.js` ainda é o servidor ativo. Routers existem mas não são
> montados.
...
const wrapSync = (fn) => (req, res, next) => {
  try {
    ...
    res
      ...
      .set(result.headers ?? {})
      .send(result.contentType === 'application/json' ? undefined : result.body);
...
router.get('/metrics', (req, res) => {
  ...
  res
    .status(result.status)
    .set('Content-Type', result.contentType ?? 'text/plain')
    .send(result.body);
});
...
**Nota importante sobre os handlers atuais**: Os handlers em `terminal/handlers/` retornam
`{ status: number; body: unknown; cors?: boolean }`, não são handlers Express nativos. O wrapper
`res.status(result.status).json(result.body)` é suficiente. Para handlers assíncronos, wrapper com
`async/await` + `next(e)`.
...
> SSE handler Express. Mais complexo — precisa gerenciar `res.writeHead` diretamente (SSE não usa
> `res.json()`). Reaproveitará `getSseClients()`, `getTerminalReplayBuffer()` de `terminal/state.js`
> (que posteriormente serão movidos).
...
> **Objetivo**: Esta é a onda mais crítica. Cria o copilot server funcional com Socket.IO. Ao final,
> o copilot server pode ser iniciado como alternativa ao `terminal/server.js`.
>
...
**Mover** a lógica de `conversation-hub/socket-ns.js` para `server/socket/hub-ns.js`. O arquivo
original em `conversation-hub/socket-ns.js` vira re-export:
...
export {
  ...
  broadcastToSession,
  broadcastGlobal,
} from '../server/socket/hub-ns.js';
```

O arquivo `server/socket/hub-ns.js` recebe o conteúdo integral dos 458 LOC originais (sem mudanças
de lógica).
...
export function createCopilotSocket(httpServer) {
  ...
  mountCopilotNamespace(io, conversationHub.orchestrator, conversationHub.store);
...
**Verificar `hub.js`**: Se já tem guarda de dupla inicialização (`if (this._initialized) return`), é
seguro. Se não tem, adicionar:
...
```js
/**
 * @deprecated L57.2 — Servidor HTTP nativo substituído por copilot/server/index.js na Onda 3.3. Este arquivo será
 *   removido na Onda 3.7. Mantido temporariamente como referência. Use `startCopilotServer()` de '../server/index.js'
 *   em vez de `createInjectServer()`.
 */
```
...
```js
/**
 * @deprecated L57.3 — Tabela de rotas substituída por Express routers em copilot/server/routes/. Este arquivo será
 *   removido na Onda 3.7.
 */
```
...
> **Objetivo**: Ativar o ConversationHub com socket.io real (não mais `initStandalone`). Dashboard
> pode se conectar via `ws://localhost:3009/socket.io` namespace `/copilot`.
>
...
Nesta onda (3.4), o foco é garantir que o namespace funciona corretamente e remover
`initStandalone()`.
...
class ConversationHub {
  ...
  initStandalone() {
    ...
    log(
      'WARN',
      '[ConversationHub] initStandalone() é deprecated. Use init() com o io do copilot server.',
    );
    this.init({});
  }
}
...
> **Objetivo**: Limpar `terminal/` de qualquer responsabilidade do servidor HTTP. Após esta onda,
> `terminal/` contem apenas: REPL, comandos, dialog, DI wiring, agent wiring.
>
> **Arquivos a mover**:
>
...
O hub session id pode ficar em `terminal/state.js` (pertence ao terminal). SSE client state vai para
`server/sse/state.js`.

**Após mover**: Atualizar imports em:

- `server/routes/sse.js` — importa de `server/sse/state.js`
- `terminal/dialog.js` — ainda pode importar de `server/sse/state.js` (cross-module OK pois dialog
  também é infra)
...
Provavelmente: `registerClearRateLimiters()` e `clearAllRateLimiters()`. Após mover, atualizar
imports em `server/middleware/rate-limiter.js`.
...
Se não importados: marcar como `@deprecated` com nota sobre Onda 3.7 (remoção). Se importados por
algo ativo: integrar ao `server/routes/sdk.js`.
