...
> **Documento**: PARTE-25A-PRE-AUDITORIA-POS-ONDA3.md **Série**: PARTE-25 (nova auditoria
> arquitetural completa) **Data**: 2026-04-13 **Base**: HEAD = `db7334a7` (Ondas 3.0–3.9 completas)
...
- `getSseClients`, `getSseCriticalClients`, `getTerminalReplayBuffer` → deveriam ser de
  `server/sse/`
...
- **Problema**: `rate-limiter-state.js` agora serve `repl.js` e `system-metrics.js` — ainda tem
  raison d'être
...
- Hub ainda usa `initStandalone()` no `terminal/index.js` (linha: `conversationHub.init()` /
  `initStandalone()`)
...
- Os re-exports são o padrão correto (Onda 3.5), mas o acoplamento persiste até a Onda 3.9 real
  (mover implementação)
...
2. **Módulos autocontidos**: cada módulo expõe API pública via `index.js`; imports externos usam
   alias
3. **Server como único transport layer**: `server/` é o único módulo que expõe HTTP/Socket.IO
4. **Terminal como UI layer**: `terminal/` não conhece HTTP; usa apenas `server/sse/` como canal de
   saída
