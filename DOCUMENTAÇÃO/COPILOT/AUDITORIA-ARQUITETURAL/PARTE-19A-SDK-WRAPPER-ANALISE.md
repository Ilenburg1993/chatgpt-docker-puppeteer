...
**99% das violações são type-only (JSDoc)** — não há imports runtime do SDK fora do wrapper. As violações são da forma:
...
- `src/copilot/tools/introspection-tools.js:157` — `_req('@github/copilot-sdk/package.json')`.version
...
**Conclusão**: Todos os 15 tipos já existem em `src/copilot/sdk/types.js` (SSOT). A migração é de referência, não de conteúdo.
...
- `session/initializer.js` — 5 (CopilotClient, CopilotSession, Tool, PermissionHandler, SystemMessageConfig)
...
1. **Centralização**: mudanças de nome de tipo no SDK precisam ser atualizadas em 1 local (`types.js`) em vez de 53 arquivos
...
**Fase 1 — Agent Core (S01–S04)**: Arquivos mais críticos e mais referenciados. Menor risco pois são typedefs locais não exportados.

**Fase 2 — Config + Tools (S05–S08)**: Tools e config são estáveis mas têm maior densidade de `ZodSchema` inline.

**Fase 3 — API + Bridges + Observability (S09–S10)**: Fronteira da API e integrações externas.

**Fase 4 — Validação + Pendentes 18C (S11, F75, F80)**: Validação final e faixas pendentes do roadmap anterior.
...
- `rg "@github/copilot-sdk" src/ -t js | grep -v "src/copilot/sdk/" | grep -v "package.json" | grep -v "^.*comment"` retorna 0 linhas
...
*Documento gerado automaticamente — 2026-07-21*
