...
**Impacto**: Se o state file corromper (JSON inválido, permissão, disco cheio), a aplicação continua sem state — pode gerar sessões duplicadas, perda de contexto de dialog loop, ou crashloop.
**Arquivo**: `agent/lifecycle/state-io.js:98`
**Fix**: Logar o erro e emitir evento de alerta.
...
**Impacto**: Perda silenciosa de writes no state file. Se a queue falhar persistentemente, o state.json fica stale e futuras reinicializações podem usar dados desatualizados.
**Arquivo**: `agent/lifecycle/state-io.js:247`
**Fix**: Logar e contar falhas; emitir `AGENT_STATE_WRITE_FAILED` no bus.
...
**Impacto**: Snapshots são usados para session rewind. Falha silenciosa significa que o agente não sabe que não tem snapshots disponíveis — pode tentar rewind sem dados.
**Arquivo**: `agent/session/snapshot.js:139` (deprecated mas ainda chamado)
...
**Impacto**: No pattern `using agent = ...`, se `stop()` falhar (sessão travada, timeout), o erro é completamente ignorado. Pode deixar recursos (WebSocket, timers, pids) orphaned.
**Arquivo**: `agent/always-alive.js:537`
...
**Impacto**: Se o `import()` dinâmico falhar por razão diferente de "módulo não encontrado" (ex: syntax error no di-container.js), o bridge agent→EventBus nunca é configurado. Todos os 80+ event types ficam sem bridge.
**Arquivo**: `agent/always-alive.js:665-746`
**Fix**: Catch seletivo — rejeitar erros que não sejam MODULE_NOT_FOUND.
...
**Impacto**: Se `todos.json` estiver corrompido (arquivo truncado, escrita parcial), `JSON.parse` lança e o `count()` retorna sem valor definido → runtime error cascading.
**Arquivo**: `tools/todo/store.js:62-63`

### C-08 — 119 Map/Set instances sem WeakMap/WeakRef e zero FinalizationRegistry

**Impacto**: Em sessões de longa duração (8h+), Maps/Sets que acumulam entries sem limpeza causam memory leak gradual. Nenhum uso de WeakMap/WeakRef em 389 arquivos.
**Evidência**: `grep 'new Map\|new Set' → 119 ocorrências`, `grep WeakMap → 0`
...
**Impacto**: Bloqueia o event loop durante leitura de arquivo. Em arquivos grandes (>1MB), pode causar stall no processamento de outras requests.
**Arquivo**: `tools/tool-factory.js:39`
...
**Impacto**: Se tools config falhar ao carregar, o agente opera com config default — tools customizadas não carregam.
**Arquivo**: `sdk/tools/state.js:38`
...
**Impacto**: Custom tools silenciosamente indisponíveis. Usuário não sabe que configurou tools que nunca carregaram.
**Arquivo**: `sdk/tools/custom.js:159`
...
**Impacto**: Quota check resiliente mas se ambas falharem, quota info fica stale indefinidamente. Sem limite de staleness.
**Arquivo**: `sdk/telemetry/quota-monitor.js:120-124`
...
**Impacto**: Se a função que cria este interval não retorna o handle para cleanup, ou se o módulo é re-importado, cria timers orphans.
**Arquivo**: `terminal/terminal-agent-wiring.js:66`
...
**Impacto**: Se a store inicializar mas falhar depois, o checkpoint timer pode continuar rodando tentando checkpointar dados inválidos.
**Arquivo**: `conversation-hub/store.js:87`

### A-08 — 178 `.on()` calls vs 55 `.off()`/`removeListener()` — 3:1 ratio

**Impacto**: Potential event listener leaks. Em sessões longas, listeners acumulam se não forem removidos em error/cleanup paths.
**Evidência**: `grep '.on(' → 178`, `grep '.off(' + '.removeListener(' → 55`
...
**Impacto**: Se JWT_SECRET não for configurado ou for inválido, Socket.IO namespace opera sem autenticação. Em produção, qualquer client pode se conectar.
...
**Impacto**: Se o body HTTP não for JSON válido (timeout, partial response), o tool handler crasha e o modelo recebe erro não informativo.
**Arquivo**: `tools/task-tools.js:49, 96, 146`
...
**Impacto**: Se CLI retornar output não-JSON (ex: error message), crash.
**Arquivo**: `bridges/gh/shared.js:52`
...
**Impacto**: Mesmo padrão — uma linha corrompida no JSONL interrompe o pipeline audit.
**Arquivo**: `audit/pipeline-audit-log.js:295`

### A-14 — `config/env.js:42-68` — 27 exports consecutivos sem JSDoc

**Impacto**: Nenhuma documentação de tipo, default ou range válido para variáveis de ambiente. Configuração incorreta é silenciosa.
**Arquivo**: `config/env.js:42-68`
...
**Impacto**: Pode retornar sessionId de uma sessão anterior (do state file) quando a sessão atual ainda está inicializando. Clientes podem operar em sessão errada.
**Arquivo**: `agent/always-alive.js:208`
...
**Impacto**: Se aliases falham ao carregar, comandos customizados do terminal não funcionam sem aviso.
**Arquivo**: `terminal/alias-store.js:49`
...
**Impacto**: Health endpoint pode reportar healthy quando subsistema está em falha.
**Arquivo**: `server/routes/health.js:49`
...
**Impacto**: Se o check for pesado (muitas métricas), 30s fixo pode ser insuficiente para completar. Sem backoff, checks acumulam.
**Arquivo**: `observability/error-alerting.js:232`

### A-20 — 51 magic numbers de timeout espalhados

**Impacto**: Timers com valores hardcoded (1000, 5000, 30000, 60000, 120000, 300000) espalhados sem constantes nomeadas. Mudança de timeout requer grep+replace.
**Evidência**: 51 ocorrências em arquivos de produção.
...
**Impacto**: Se state write falhar, o dialog loop continua mas o state file fica inconsistente. Próximo restart pode resumir em estado errado.
**Arquivo**: `agent/dialog/loop-manager.js:402, 430, 476`
...
**Impacto**: Orquestração de hub com erros apenas logados. Se o orquestrador falhar, sessões podem ficar em estado inconsistente.
**Arquivo**: `conversation-hub/orchestrator.js:258`
...
**Impacto**: TypeScript strict mode pode inferir tipo errado. Inconsistência com padrão do projeto (`/** @type {any} */ e`).

### M-02 — 8 blocos `catch {}` vazios
**Lista**: state-io.js:98, snapshot.js:139,152, tools/state.js:38, tools/custom.js:159, alias-store.js:49, quota-monitor.js:120,124
...
**Impacto**: Regex patterns para bloquear comandos perigosos podem ser bypassados via encoding, aliases, ou builtins.
...
```
**Impacto**: `core/` deveria ser leaf node. Importar de `events/` cria dependência circular potencial.
...
**Impacto**: Apenas webhooks e sessions têm Zod validation (Onda 6.0). Demais POST/PUT aceitam payloads arbitrários.
...
**Impacto**: Complexidade de controle de fluxo dificulta debugging. Potential unhandled rejection se resolve/reject thrown.
...
*76 bugs categorizados. Próximo: 02-GAPS-FUNCIONAIS.md*
