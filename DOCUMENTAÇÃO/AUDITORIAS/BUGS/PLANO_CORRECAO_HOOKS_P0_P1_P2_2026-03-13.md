...
Eliminar primeiro os riscos de bloqueio indevido/encerramento incorreto de sessão (P0),
normalizar contrato e métricas (P1), e finalizar com hardening de qualidade/shell hygiene (P2).
...
- **P0-H049 — Recovery contaminado por checkpoint sintético**
	...
	- Evidência: arquivo real em runtime `.github/hooks/checkpoints/sess_test123_turn10.json` foi eleito como
		sessão anterior, gerando alerta falso de encerramento incorreto.
...
- **P0-H050 — Ausência de sanidade em seleção de checkpoint**
	...
	- Impacto: qualquer fixture residual pode contaminar recovery e induzir ações erradas no começo da sessão.
...
- `pre-tool-use.sh`: guard de `git push` virou telemetria (`gitPush_detected` / `gitPush_requires_template_g`) sem bloqueio por close_key.
...
- `smoke-test.sh --quiet`: **1 falha pré-existente** (`session-start.sh` classificado no teste de guard), sem indício de regressão da P0.
...
1. **H001** — `agent-stop.sh`: remover bloqueio global por `close_key_validated=false` em fim de TURN.
...
- Ajuste de gating em `agent-stop.sh` para ativar regras de close apenas quando houver intenção explícita de encerramento de SESSION.
- Guard de push migrado para lógica de autorização própria (Template G), sem travar fluxo normal de entrega.
...
- cenário manual: turno normal (sem intenção de close), push normal, fechamento com Template F válido.
- `bash .github/hooks/test-level1-detect.sh` com cenário anti-contaminação (synthetic checkpoint ignored).
...
1. **H002/H003/H012** — alinhar protocolo entre scripts e docs (`session-start.sh`, `.github/AGENTS.md`, instruções).
