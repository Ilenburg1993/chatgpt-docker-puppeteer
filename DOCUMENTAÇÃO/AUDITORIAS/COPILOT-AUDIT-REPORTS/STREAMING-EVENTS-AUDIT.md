...
| 12.2     | ✅ DONE  | Replay buffer no terminal + Last-Event-ID parsing         |
| 13.1     | ✅ FEITO | `standardizeSsePayload()` em `sse-utils.js`               |
| 13.2     | ✅ DONE  | Aplicar `standardizeSsePayload()` nos 4 endpoints Express |
| 13.3     | ✅ FEITO | `maxContentChars` option no `createSseWriter`             |
| 14.1     | 📋 DOC   | Duplicação bridge↔terminal documentada (overhead O(1))    |
| 14.2     | ✅ DONE  | Unificar nomes com mapping + alias layer                  |
| 15.1     | ✅ DONE  | Separar emitSocket de broadcastSse                        |
| 15.2     | ✅ DONE  | EventFanout inter-processo (plugável)                     |

> **NOTA**: Todas as fases são OBRIGATÓRIAS — nenhuma fase é opcional ou adiada.

**Testes**: 2049/0 pass. Lint: 0 erros.

---

## §16 — PR Consumption vs Dialog Loop: Classificação de Eventos

### 16.1 Resumo executivo

O sistema utiliza dois modos de interação com o SDK:

1. **`sendMessage()`** — consome 1 Premium Request (PR) por chamada. Eventos relacionados: `task.*`
2. **`dialogTurn()`** — NÃO consome PR. Eventos relacionados: `dialog.*`

O dialog loop é o modo padrão do Terminal LLM-B para interação contínua sem custo de PR.

### 16.2 Classificação por grupo

| Grupo           | Consome PR? | Mecanismo       | Eventos                                                                                           |
| --------------- | ----------- | --------------- | ------------------------------------------------------------------------------------------------- |
| **task.\***     | ⚠️ SIM      | `sendMessage()` | queued, started, completed, error, delta, reasoning                                               |
| **dialog.\***   | ✅ NÃO      | `dialogTurn()`  | ready, reply, stopped, stalled, paused, resumed, loop.changed, turn_start, turn_end, turn_timeout |
| **session.\***  | ✅ NÃO      | Observabilidade | compaction\_\*, fatal, usage, token_budget_warning, mode_changed, history_synced, info, etc.      |
| **tool.\***     | ✅ NÃO      | Dentro de turno | execution_start, execution_complete, execution_progress                                           |
| **pr.\***       | ℹ️ Tracking | Após consumo    | consumed, fallback_model                                                                          |
| **question.\*** | ✅ NÃO      | Estado          | pending, answered                                                                                 |

### 16.3 Exports disponíveis

```js
import { PR_CONSUMING_EVENTS, DIALOG_LOOP_EVENTS } from '#copilot/core';
// PR_CONSUMING_EVENTS: Set{'task.started', 'task.completed', 'task.error', 'pr.consumed'}
// DIALOG_LOOP_EVENTS: Set{'dialog.ready', 'dialog.reply', ...todos os dialog.*}
```

### 16.4 Regra de ouro

> **NUNCA** usar `sendMessage()` quando o dialog loop está ativo. O guard em `always-alive.js`
> (`if (this.#dialogLoop.active)`) bloqueia a chamada e redireciona para `sendDialogTurn()`.

### 16.5 Commit de implementação

- `5780d992`: Fases 12.2–15.2, incluindo PR annotations e
  `PR_CONSUMING_EVENTS`/`DIALOG_LOOP_EVENTS`.
