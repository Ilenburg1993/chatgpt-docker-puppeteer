...
4. [Configuração (ecosystem.config.js)](#configuração-ecosystemconfigjs)
...
1. **Camada 1: Configuração Forçada** (`ecosystem.config.js`)
...
```
┌─────────────────────────────────────────────────────────────┐
│ CAMADA 1: CONFIGURAÇÃO (ecosystem.config.js)                │
│ ──────────────────────────────────────────────────────────  │
...
**Arquitetura**: Veja `DOCUMENTAÇÃO/CONNECTION_ARCHITECTURE/` (2,600+ linhas)
...
# Validação
make pm2-validate    # Valida ecosystem.config.js
```
...
pm2 start ecosystem.config.js --only chrome-proxy
```
