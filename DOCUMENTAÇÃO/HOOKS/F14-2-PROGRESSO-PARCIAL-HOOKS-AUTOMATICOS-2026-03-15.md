...
**Fase**: F14.2 (**concluída**) **Snapshot machine-readable**:
`.github/hooks/state/f14-auto-hook-entry-lib-status.json`

## Escopo consolidado da F14.2

Migração concluída de lógica de domínio (script -> entry-lib) para os 8 hooks automáticos
aplicáveis:
...
`agentStop` permaneceu como hook de referência com entry-lib já existente
(`hooks-lib/agent-stop-lib.sh`), conforme estratégia da trilha F15.2.

## Alterações estruturais aplicadas

- Scripts automáticos no padrão **entrypoint fino** (bootstrap + source common + source entry-lib +
  dispatch único).
...
A F14.2 atende o critério de saída de migração de lógica para libs dedicadas sem mudança de contrato
externo dos hooks automáticos.
