...
> **Status**: Canônico | **Última atualização**: 2026-03-09 **Schema verificado empiricamente** em
> 2026-03-09 via inspeção de `raw-input.jsonl` e `raw-post-input.jsonl`.
...
- `result_type`:
  ...
  - `"failure"` — resposta contém padrão de erro (`Error:`, `ENOENT`, `fatal:`, etc.) → incrementa
    `failures_detected`
...
- Emitido por `start-section.sh` (chamada manual do agente) ou por `session-start.sh` (seção padrão
  `"início"`)
...
- Emitido por `section-end.sh` (manual), `start-section.sh` (auto-close da anterior) ou
  `session-end.sh` (reason: `session_ended`)
...
- Resposta completa é armazenada em `session-context.json` →
...
1. **`timestamp` em `preToolUse`/`postToolUse`**: formato ISO 8601 com milissegundos **`timestamp`
   em `agentStop`/`sessionStart`**: alguns eventos usam epoch string

2. **`result_type` sem campo explícito no payload**: determinado por heurística (tool_response vazia
   → `"unknown"`, não vazia → `"success"`)

3. **`session_id` em preToolUse**: vem do payload do Copilot (campo `.session_id`) **`session_id` em
   error-occurred**: lido de `session-context.json` (pode ser stale se for erro de outra sessão)

4. **`userPromptSubmitted` é raro** no audit.jsonl: o hook `userPromptSubmitted` só dispara quando o
   Copilot recebe foco. Em sessões longas, pode haver múltiplos turnos sem nenhum
   `userPromptSubmitted` — por isso existem as Estratégias 2 e 3 no protocolo de autorização.

---

_Mantido pelo Modo Arquiteto. Para atualizar, editar este arquivo e commitar no branch principal._
