...
Esta visão integrada segue a mesma topologia do baseline oficial: `src/main.js` como bootstrap,
`src/nerv/` como barramento, `src/kernel/` e `src/orchestrator/` como núcleo de decisão,
`src/driver/` como atuador browser, `src/infra/` como sustentação e `src/server/` +
`src/dashboard-ui/` como superfícies externas.
...
- [Documentação de Auditoria](DOCUMENTAÇÃO/AUDITORIAS/BUGS/CODEX_AUDIT_TRACKER.md)
- [Plano Mestre do Audit Agent](DOCUMENTAÇÃO/AUDITORIAS/BUGS/CODEX_AUDIT_AGENT_MASTER_PLAN.md)
