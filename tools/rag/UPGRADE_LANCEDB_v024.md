...
**Why:** Discard irrelevant results (distance > 0.8)
**Code Change:** Add `.distanceRange(minDist, maxDist)` to search
...
**Why:** Enable text-based search (keywords, phrases)
**Code Change:** Create FTS index on `text` column
...
const results = await table
    .search()
    .fullText('async function', { column: 'text' })
    .limit(10);
```
...
**Why:** Combine semantic similarity + keyword matching
**Code Change:** Use both FTS and vector in single query
...
**Why:** Diagnose slow queries, optimize performance
**Code Change:** Add debug mode to search
...
**Why:** Faster lookups by chunk_id (SHA256 hash)
**Code Change:** Create scalar index
